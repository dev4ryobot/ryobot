...
async def setup_database():
    ...
    # 1. Migrate ryo_inventory column names
    try:
        has_inventory = await run_query("""
            SELECT 1 FROM information_schema.tables WHERE table_name = 'ryo_inventory'
        """, fetchone=True)
        if has_inventory:
            # Check if instance_id column exists
            has_instance_id = await run_query("""
                SELECT COLUMN_NAME 
                FROM INFORMATION_SCHEMA.COLUMNS 
                WHERE TABLE_NAME = 'ryo_inventory' AND COLUMN_NAME = 'instance_id';
            """, fetchone=True)
            if not has_instance_id:
                ...
                """, fetchone=True)
                if has_id_col:
                    await run_query("ALTER TABLE ryo_inventory RENAME COLUMN id TO instance_id;")
                    print("[Migration] Renamed 'id' to 'instance_id' in ryo_inventory")
                else:
                    await run_query("ALTER TABLE ryo_inventory ADD COLUMN instance_id SERIAL PRIMARY KEY;")
                    print("[Migration] Added 'instance_id' SERIAL PRIMARY KEY to ryo_inventory")
    except Exception as e:
        print(f"[Migration Warning] Failed ryo_inventory schema migration: {e}")
    await run_query("""
    ...
    # Ensure quest_id in ryo_quests and ryo_user_quests is VARCHAR(100) and compatible
    try:
        fk_name_row = await run_query("""
            ...
            WHERE tc.constraint_type = 'FOREIGN KEY' 
              AND tc.table_name = 'ryo_user_quests' 
              AND kcu.column_name = 'quest_id';
        """, fetchone=True)
        if fk_name_row:
            fk_name = fk_name_row[0]
            await run_query(f"ALTER TABLE ryo_user_quests DROP CONSTRAINT IF EXISTS {fk_name};")
    except Exception as e:
        print(f"[Migration Warning] Failed dropping foreign key constraint: {e}")

    try:
        has_quests_table = await run_query("""
            SELECT 1 FROM information_schema.tables WHERE table_name = 'ryo_quests'
        """, fetchone=True)
        if has_quests_table:
            await run_query("ALTER TABLE ryo_quests ALTER COLUMN quest_id TYPE VARCHAR(100);")
    except Exception as e:
        print(f"[Migration Warning] Failed altering ryo_quests.quest_id: {e}")

    try:
        has_user_quests_table = await run_query("""
            SELECT 1 FROM information_schema.tables WHERE table_name = 'ryo_user_quests'
        """, fetchone=True)
        if has_user_quests_table:
            await run_query("ALTER TABLE ryo_user_quests ALTER COLUMN quest_id TYPE VARCHAR(100);")
    except Exception as e:
        print(f"[Migration Warning] Failed altering ryo_user_quests.quest_id: {e}")
    ...
    try:
        fk_exists = await run_query("""
            SELECT 1 
            FROM information_schema.table_constraints 
            WHERE constraint_type = 'FOREIGN KEY' 
              AND table_name = 'ryo_user_quests' 
              AND constraint_name = 'ryo_user_quests_quest_id_fkey';
        """, fetchone=True)
        if not fk_exists:
            # Delete orphan records referencing a non-existent quest_id to satisfy foreign key requirement
            await run_query("DELETE FROM ryo_user_quests WHERE quest_id NOT IN (SELECT quest_id FROM ryo_quests);")
            await run_query("ALTER TABLE ryo_user_quests ADD CONSTRAINT ryo_user_quests_quest_id_fkey FOREIGN KEY (quest_id) REFERENCES ryo_quests(quest_id) ON DELETE CASCADE;")
    except Exception as e:
        print(f"[Migration Warning] Failed adding back foreign key constraint: {e}")
...
class RyoBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        @self.tree.command(name="sync", description="Developer only: Sync global app commands")
        async def sync_alias(interaction: discord.Interaction):
            if interaction.user.id != DEV_ID:
                await interaction.response.send_message("❌ Access denied.", ephemeral=True)
                return
            await interaction.response.defer(ephemeral=True)
        ...
        # Automatically sync commands to Discord globally on startup so they always exist and are up to date!
        try:
            print("[Startup] Auto-syncing command tree globally...")
            synced = await self.tree.sync()
            print(f"[Startup] Successfully auto-synced {len(synced)} command(s) globally.")
        except Exception as e:
            print(f"[Startup Warning] Failed to auto-sync commands on startup: {e}")
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.CommandOnCooldown):
        ...
        try:
            error_msg = "❌ An unexpected error occurred while processing this command."
            if interaction.user.id == DEV_ID:
                # Include exact error for developer
                orig = getattr(error, "original", error)
                tb_lines = traceback.format_exception(type(orig), orig, orig.__traceback__)
...
async def draw_cmd(interaction: discord.Interaction, pack_name: str = None):
    ...
    pack_id, p_name, price_paint_raw, price_glue_raw, required_role_id, cooldown_seconds = pack
    price_paint = price_paint_raw or 0
    price_glue = price_glue_raw or 0
