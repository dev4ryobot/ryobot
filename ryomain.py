...
CHALKFLOWER_EMOJI = "<a:chalkflower:1515178945669107862>"
# Rarity Emojis
...
async def setup_database():
    ...
    # 0. SCHEMA MIGRATION: Ensure ryo_inventory has 'id' column
    try:
        table_exists = await run_query("""
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_name = 'ryo_inventory'
            );
        """, fetchone=True)
        if table_exists and table_exists[0]:
            has_id_col = await run_query("""
                SELECT COLUMN_NAME
                FROM INFORMATION_SCHEMA.COLUMNS
                WHERE TABLE_NAME = 'ryo_inventory' AND COLUMN_NAME = 'id';
            """, fetchone=True)
            if not has_id_col:
                print("[Migration] ryo_inventory is missing 'id' column. Adding id SERIAL...")
                await run_query("ALTER TABLE ryo_inventory ADD COLUMN id SERIAL;")
                print("[Migration] Added 'id' column to ryo_inventory successfully.")
    except Exception as e:
        print(f"[Migration Warning] Failed ryo_inventory 'id' column check/migration: {e}")

    # 0b. SCHEMA MIGRATION: Migrating from previous agent's unified/custom schema if it exists,
    # ensuring no user data is lost, and restoring original ryo_user_binders, ryo_binder_slots,
    # ryo_binder_inventory, and ryo_binder_templates.
    try:
        # Check if the unified ryo_binder_instances table exists
        has_instances = await run_query("""
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_name = 'ryo_binder_instances'
            );
        """, fetchone=True)
        if has_instances and has_instances[0]:
            print("[Migration] Detected leftover 'ryo_binder_instances' from unified schema. Initiating safe data-preserving migration...")
            
            # 1. Create ryo_binder_templates first if not exists
            await run_query("CREATE TABLE IF NOT EXISTS ryo_binder_templates (id TEXT PRIMARY KEY, name TEXT, image_url TEXT, paint_price INT DEFAULT 0, glue_price INT DEFAULT 0);")
            
            # 2. Copy from ryo_binder_shop_items if it exists
            has_shop_items = await run_query("""
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_name = 'ryo_binder_shop_items'
                );
            """, fetchone=True)
            if has_shop_items and has_shop_items[0]:
                await run_query("""
                    INSERT INTO ryo_binder_templates (id, name, image_url, paint_price, glue_price)
                    SELECT purchase_id, name, image_url, paint_price, glue_price FROM ryo_binder_shop_items
                    ON CONFLICT (id) DO NOTHING;
                """)
                print("[Migration] Migrated shop items to ryo_binder_templates.")
            
            # 3. Create ryo_binder_inventory first if not exists
            await run_query("CREATE TABLE IF NOT EXISTS ryo_binder_inventory (user_id TEXT, binder_id TEXT, quantity INT DEFAULT 1, PRIMARY KEY (user_id, binder_id));")
            
            # 4. If ryo_user_binders currently has the 'purchase_id' column, it's the old inventory table!
            has_purchase_id_col = await run_query("""
                SELECT EXISTS (
                    SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
                    WHERE TABLE_NAME = 'ryo_user_binders' AND COLUMN_NAME = 'purchase_id'
                );
            """, fetchone=True)
            if has_purchase_id_col and has_purchase_id_col[0]:
                await run_query("""
                    INSERT INTO ryo_binder_inventory (user_id, binder_id, quantity)
                    SELECT user_id, purchase_id, 1 FROM ryo_user_binders
                    ON CONFLICT (user_id, binder_id) DO NOTHING;
                """)
                print("[Migration] Migrated purchased binder inventory from old ryo_user_binders.")
                # Drop the old ryo_user_binders since it has the wrong columns and needs to be recreated as the instance table
                await run_query("DROP TABLE IF EXISTS ryo_user_binders CASCADE;")
            
            # 5. Backup the slots before dropping/recreating ryo_binder_slots to clear the old foreign keys
            await run_query("DROP TABLE IF EXISTS ryo_binder_slots_backup CASCADE;")
            await run_query("CREATE TABLE ryo_binder_slots_backup AS SELECT * FROM ryo_binder_slots;")
            await run_query("DROP TABLE IF EXISTS ryo_binder_slots CASCADE;")
            
            # 6. Recreate ryo_user_binders with the original schema
            await run_query("CREATE TABLE IF NOT EXISTS ryo_user_binders (id SERIAL PRIMARY KEY, user_id TEXT, binder_id TEXT, name TEXT, UNIQUE(user_id, name));")
            
            # 7. Migrate instances from ryo_binder_instances to ryo_user_binders
            await run_query("""
                INSERT INTO ryo_user_binders (id, user_id, binder_id, name)
                SELECT binder_instance_id, user_id, 'classic', 'Binder ' || binder_number FROM ryo_binder_instances
                ON CONFLICT (user_id, name) DO NOTHING;
            """)
            print("[Migration] Migrated created binder instances to ryo_user_binders.")
            
            # 8. Recreate ryo_binder_slots with original schema
            await run_query("CREATE TABLE IF NOT EXISTS ryo_binder_slots (binder_instance_id INT, slot INT, card_id TEXT, PRIMARY KEY (binder_instance_id, slot));")
            
            # 9. Restore slot values from backup
            await run_query("""
                INSERT INTO ryo_binder_slots (binder_instance_id, slot, card_id)
                SELECT binder_instance_id, slot, card_id FROM ryo_binder_slots_backup
                ON CONFLICT DO NOTHING;
            """)
            print("[Migration] Restored slots from backup.")
            
            # 10. Clean up obsolete tables
            await run_query("DROP TABLE IF EXISTS ryo_binder_instances CASCADE;")
            await run_query("DROP TABLE IF EXISTS ryo_binder_shop_items CASCADE;")
            await run_query("DROP TABLE IF EXISTS ryo_binder_slots_backup CASCADE;")
            print("[Migration] Completed database migration successfully!")
            
    except Exception as e:
        print(f"[Migration Warning] Failed ryo_binder migration: {e}")

    # 0c. SCHEMA MIGRATION: Ensure ryo_user_binders has correct columns (id, binder_id, name) even if we didn't go through the above block
    try:
        table_exists = await run_query("""
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_name = 'ryo_user_binders'
            );
        """, fetchone=True)
        if table_exists and table_exists[0]:
            # id column
            has_id = await run_query("""
                SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                WHERE TABLE_NAME = 'ryo_user_binders' AND COLUMN_NAME = 'id';
            """, fetchone=True)
            if not has_id:
                print("[Migration] ryo_user_binders is missing 'id' column. Adding id SERIAL...")
                await run_query("ALTER TABLE ryo_user_binders ADD COLUMN id SERIAL;")
                print("[Migration] Added 'id' column to ryo_user_binders successfully.")
            
            # binder_id column
            has_binder_id = await run_query("""
                SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                WHERE TABLE_NAME = 'ryo_user_binders' AND COLUMN_NAME = 'binder_id';
            """, fetchone=True)
            if not has_binder_id:
                print("[Migration] ryo_user_binders is missing 'binder_id' column. Adding binder_id TEXT...")
                await run_query("ALTER TABLE ryo_user_binders ADD COLUMN binder_id TEXT;")
                print("[Migration] Added 'binder_id' column to ryo_user_binders successfully.")
                
            # name column
            has_name = await run_query("""
                SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                WHERE TABLE_NAME = 'ryo_user_binders' AND COLUMN_NAME = 'name';
            """, fetchone=True)
            if not has_name:
                print("[Migration] ryo_user_binders is missing 'name' column. Adding name TEXT...")
                await run_query("ALTER TABLE ryo_user_binders ADD COLUMN name TEXT;")
                print("[Migration] Added 'name' column to ryo_user_binders successfully.")
    except Exception as e:
        print(f"[Migration Warning] Failed ryo_user_binders check/migration: {e}")
    ...
    queries = [
        ...
        "ALTER TABLE ryo_users ADD COLUMN IF NOT EXISTS last_guess_streak INT DEFAULT 0;",
        ...
        "ALTER TABLE ryo_shop_packs ADD COLUMN IF NOT EXISTS glue_price INT DEFAULT 0;",
        "ALTER TABLE ryo_shop_packs ADD COLUMN IF NOT EXISTS cooldown INT DEFAULT 24;",
        "ALTER TABLE ryo_shop_packs ADD COLUMN IF NOT EXISTS no_of_cards INT DEFAULT 3;",
        "ALTER TABLE ryo_shop_packs ADD COLUMN IF NOT EXISTS rarity TEXT DEFAULT '1,2,3,4,5';",
        "ALTER TABLE ryo_shop_packs ADD COLUMN IF NOT EXISTS era TEXT;",
        ...
        "CREATE TABLE IF NOT EXISTS ryo_binder_templates (id TEXT PRIMARY KEY, name TEXT, image_url TEXT, paint_price INT DEFAULT 0, glue_price INT DEFAULT 0, drop_emoji TEXT, type TEXT DEFAULT 'Colors');",
        "ALTER TABLE ryo_binder_templates ADD COLUMN IF NOT EXISTS drop_emoji TEXT;",
        "ALTER TABLE ryo_binder_templates ADD COLUMN IF NOT EXISTS type TEXT DEFAULT 'Colors';",
        "CREATE TABLE IF NOT EXISTS ryo_binder_inventory (user_id TEXT, binder_id TEXT, quantity INT DEFAULT 1, PRIMARY KEY (user_id, binder_id));",
        "CREATE TABLE IF NOT EXISTS ryo_user_binders (id SERIAL PRIMARY KEY, user_id TEXT, binder_id TEXT, name TEXT, UNIQUE(user_id, name));",
        "CREATE TABLE IF NOT EXISTS ryo_binder_slots (binder_instance_id INT, slot INT, card_id TEXT, PRIMARY KEY (binder_instance_id, slot));",
...
def add_rounded_corners(im, radius=12):
...
def draw_empty_slot(draw, x, y, card_w, card_h, slot):
    ...
    draw.rounded_rectangle(box, radius=20, fill=(235, 230, 245, 180), outline=(140, 110, 170), width=5)
    ...
    tx = x + (card_w - tw) // 2
    ty = y + (card_h - th) // 2 - 35
    draw.text((tx, ty), slot_text, fill=(110, 80, 140), font=font)
    ...
    draw.text((ptx, pty), plus_text, fill=(110, 80, 140), font=plus_font)

async def get_binder_slots(binder_instance_id):
    rows = await run_query("SELECT slot, card_id FROM ryo_binder_slots WHERE binder_instance_id::text = %s::text", (str(binder_instance_id),), fetchall=True) or []
    slots_map = {row[0]: row[1] for row in rows}
    
    card_ids = list(set([row[1] for row in rows if row[1]]))
    card_details = {}
    if card_ids:
        placeholders = ", ".join(["%s"] * len(card_ids))
        card_rows = await run_query(f"SELECT card_id, image_url, member_name, group_name, era, rarity FROM ryo_cards WHERE LOWER(card_id) IN ({placeholders})", [cid.lower() for cid in card_ids], fetchall=True) or []
        card_details = {r[0].lower(): r for r in card_rows}
        
    return slots_map, card_details

async def generate_binder_image_buffer(binder_instance_id, bg_url, slots_map, card_details):
    try:
    ...
    canvas = None
    if bg_url and bg_url.lower().startswith("http"):
        try:
            session = await get_session()
            headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"}
            async with session.get(get_raw_url(bg_url.strip()), timeout=12, headers=headers) as resp:
                if resp.status == 200:
                    bg_data = await resp.read()
                    canvas = Image.open(io.BytesIO(bg_data))
                    canvas = canvas.convert("RGBA")
                    canvas = canvas.resize((2048, 1536), resample_filter)
        except Exception as e:
            print(f"Error loading custom binder background {bg_url}: {e}")

    if not canvas:
        bg_path = "/src/assets/images/binder_background_1782549906509.jpg"
        try:
            canvas = Image.open(bg_path)
            canvas = canvas.convert("RGBA")
            canvas = canvas.resize((2048, 1536), resample_filter)
        except Exception as e:
            print(f"Error loading fallback binder background: {e}")
            canvas = Image.new('RGBA', (2048, 1536), (45, 30, 60))
            draw = ImageDraw.Draw(canvas)
            draw.rounded_rectangle([60, 60, 990, 1476], radius=30, fill=(245, 240, 250))
            draw.rounded_rectangle([1058, 60, 1988, 1476], radius=30, fill=(245, 240, 250))
            for y in range(150, 1450, 250):
                draw.rounded_rectangle([995, y, 1053, y+35], radius=15, fill=(100, 100, 100))
    ...
    card_w, card_h = 400, 534
    session = await get_session()
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"}
    
    async def load_one_card(slot, card_id):
        if not card_id:
            return None
            ...
            async with session.get(target_url, timeout=12, headers=headers) as resp:
                if resp.status == 200:
                    ...
                    with Image.open(io.BytesIO(data)) as card_img:
                        ...
                        # Scale to fit while maintaining aspect ratio, then center on a transparent canvas
                        ratio = min(card_w / card_img.width, card_h / card_img.height)
                        new_w = int(card_img.width * ratio)
                        new_h = int(card_img.height * ratio)
                        scaled = card_img.resize((new_w, new_h), resample_filter)
                        transparent_bg = Image.new('RGBA', (card_w, card_h), (0, 0, 0, 0))
                        offset_x = (card_w - new_w) // 2
                        offset_y = (card_h - new_h) // 2
                        transparent_bg.paste(scaled, (offset_x, offset_y), scaled)
                        resized = transparent_bg
                            
                        rounded = add_rounded_corners(resized, radius=20)
                        return (slot, rounded)
        except Exception as e:
            print(f"Error loading card in slot {slot}: {e}")
        return None
    ...
    for slot_num in range(1, 9):
        if slot_num in slots_map:
            ...
            if cid:
                tasks.append(load_one_card(slot_num, cid))
...
class BinderDropdownSelect(discord.ui.Select):
    def __init__(self, binders, owner_id, owner_name):
        options = []
        for instance_id, name, bid, bg_url in binders[:25]:
            options.append(discord.SelectOption(
                label=name,
                value=str(instance_id),
                description=f"Theme: {bid}"
            ))
        ...
        super().__init__(
            ...
            max_values=1,
            options=options,
            custom_id="binder_dropdown_select"
        )
        self.binders_dict = {str(b[0]): b for b in binders}
        self.owner_id = owner_id
        self.owner_name = owner_name

    async def callback(self, interaction: discord.Interaction):
        instance_id_str = self.values[0]
        selected_binder = self.binders_dict.get(instance_id_str)
        if not selected_binder:
            return await interaction.response.send_message("❌ Binder not found.", ephemeral=True)
            
        await interaction.response.defer()
        
        instance_id, bname, bid, bg_url = selected_binder
        slots_map, card_details = await get_binder_slots(instance_id)
        
        try:
            buffer = await generate_binder_image_buffer(instance_id, bg_url, slots_map, card_details)
            embed = discord.Embed(
                title=f"📖 {self.owner_name}'s Binder: {bname}",
                description=f"**Theme**: `{bid}`\nUse `/binderadd`, `/binderremove`, or `/binderclear` to customize.",
                color=RYO_COLOR
            )
            file = discord.File(buffer, filename="binder.png")
            embed.set_image(url="attachment://binder.png")
            embed.set_footer(text="RyoBot Binder System", icon_url=TEDDY_ICON_URL)
            ...
            print(f"Error switching binder view: {e}")
            await interaction.followup.send("⚠️ Failed to load binder view.", ephemeral=True)

class BinderShowView(discord.ui.View):
    def __init__(self, binders, owner_id, owner_name):
        super().__init__(timeout=180)
        self.add_item(BinderDropdownSelect(binders, owner_id, owner_name))

@bot.tree.command(name="bindershow", description="Show off your card binders")
@app_commands.describe(
    user="The player whose binder showcase you want to view",
    binder_name="Optional specific binder name to display directly"
)
async def bindershow_cmd(
    ...
    binders = await run_query(
        """
        SELECT b.id, b.name, b.binder_id, t.image_url 
        FROM ryo_user_binders b
        LEFT JOIN ryo_binder_templates t ON b.binder_id = t.id
        WHERE b.user_id = %s
        ORDER BY b.id
        """,
        (uid,),
    ...
    if not binders:
        if target.id == interaction.user.id:
            return await interaction.followup.send("❌ You don't have any active binders yet! Use `/bindercreate` to create one, or `/bindershop` to buy designs.")
        else:
            return await interaction.followup.send(f"❌ `{target.display_name}` doesn't have any active binders yet.")
            
    selected_binder = None
    if binder_name:
        for b in binders:
            if b[1].lower() == binder_name.strip().lower():
                selected_binder = b
                break
        if not selected_binder:
            return await interaction.followup.send(f"❌ No binder named `{binder_name}` was found for `{target.display_name}`.")
    else:
        selected_binder = binders[0]
    ...
    try:
        buffer = await generate_binder_image_buffer(instance_id, bg_url, slots_map, card_details)
        embed = discord.Embed(
            title=f"📖 {target.display_name}'s Binder: {bname}",
            description=f"**Theme**: `{bid}`\nUse `/binderadd`, `/binderremove`, or `/binderclear` to customize.",
            color=RYO_COLOR
        )
        file = discord.File(buffer, filename="binder.png")
        ...
        if len(binders) > 1:
            view = BinderShowView(binders, uid, target.display_name)
...
async def binder_type_autocomplete(interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
    choices = ["Colors", "Theme"]
    return [app_commands.Choice(name=c, value=c) for c in choices if current.lower() in c.lower()]

@bot.tree.command(name="addbinder", description="Add a new binder template to the shop (STAFF/MOD ONLY)")
@app_commands.describe(
    purchase_id="Unique purchase ID for the binder template",
    name="Name of the binder template",
    drop_emoji="Emoji shown next to the binder name in dropdowns (e.g. 🎨)",
    type="Category type — Colors or Theme",
    image_url="Background image URL for the binder template",
    paint_price="Price in Paint (default 0)",
    glue_price="Price in Glue (default 0)"
)
@app_commands.autocomplete(type=binder_type_autocomplete)
async def add_binder_cmd(
    interaction: discord.Interaction,
    purchase_id: str,
    name: str,
    drop_emoji: str,
    ...
    await interaction.response.defer(ephemeral=True)
    clean_id = purchase_id.strip().lower()
    clean_url = image_url.strip()
    clean_type = type.strip()

    if clean_type not in ["Colors", "Theme"]:
        return await interaction.followup.send("❌ Type must be either `Colors` or `Theme`.", ephemeral=True)

    if clean_url.lower() != "local_fallback" and not clean_url.lower().startswith("http"):
        return await interaction.followup.send("❌ Image URL must start with 'http://' or 'https://'.", ephemeral=True)

    await run_query(
        """
        INSERT INTO ryo_binder_templates (id, name, image_url, paint_price, glue_price, drop_emoji, type)
        VALUES (%s, %s, %s, %s, %s, %s, %s)
        ON CONFLICT (id) DO UPDATE SET
            name = EXCLUDED.name,
            image_url = EXCLUDED.image_url,
            paint_price = EXCLUDED.paint_price,
            glue_price = EXCLUDED.glue_price,
            drop_emoji = EXCLUDED.drop_emoji,
            type = EXCLUDED.type
        """,
        (clean_id, name.strip(), clean_url, paint_price, glue_price, drop_emoji.strip(), clean_type)
    )
    await interaction.followup.send(
        f"✅ Successfully added/updated binder template `{clean_id}` ({name}) [{clean_type}] in the shop!",
        ephemeral=True
    )

@bot.tree.command(name="deletebinder", description="Delete a binder template from the shop (STAFF/MOD ONLY)")
@app_commands.describe(id="The ID of the binder template to delete")
async def delete_binder_cmd(interaction: discord.Interaction, id: str):
    if not (is_staff(interaction) or is_mod(interaction)):
        return await interaction.response.send_message("❌ Unauthorized", ephemeral=True)
    ...
    exists = await run_query("SELECT 1 FROM ryo_binder_templates WHERE id = %s", (clean_id,), fetchone=True)
    if not exists:
        return await interaction.followup.send(f"❌ Binder template `{clean_id}` does not exist.", ephemeral=True)
        
    await run_query("DELETE FROM ryo_binder_templates WHERE id = %s", (clean_id,))
    await interaction.followup.send(f"✅ Deleted binder template `{clean_id}` from the shop.", ephemeral=True)

class BinderCategorySelect(discord.ui.Select):
    def __init__(self, items, category, shop_view, row_num):
        # items: list of (id, name, drop_emoji)
        options = []
        for tid, tname, drop_emoji in items[:25]:
            option = discord.SelectOption(label=tname[:100], value=tid)
            if drop_emoji:
                try:
                    option.emoji = drop_emoji
                except Exception:
                    pass
            options.append(option)

        super().__init__(
            placeholder=f"{category} binders...",
            min_values=1,
            max_values=1,
            options=options,
            custom_id=f"bindershop_{category.lower()}_{shop_view.user_id}",
            row=row_num
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.shop_view.user_id:
            return await interaction.response.send_message(
                "❌ This is not your shop menu. Use `/bindershop` to open your own!", ephemeral=True
            )
        selected_id = self.values[0]
        for i, row in enumerate(self.shop_view.templates):
            if row[0] == selected_id:
                self.shop_view.current_page = i
                break
        self.shop_view._build_components()
        await interaction.response.edit_message(embed=self.shop_view.build_embed(), view=self.shop_view)


class BinderNavButton(discord.ui.Button):
    def __init__(self, emoji_str, action, shop_view, disabled=False):
        super().__init__(
            emoji=discord.PartialEmoji.from_str(emoji_str),
            style=discord.ButtonStyle.secondary,
            custom_id=f"bindernav_{action}_{shop_view.user_id}",
            disabled=disabled,
            row=2
        )
        self.action = action
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        total = len(self.shop_view.templates)
        if self.action == "far_left":
            self.shop_view.current_page = 0
        elif self.action == "left":
            self.shop_view.current_page = max(0, self.shop_view.current_page - 1)
        elif self.action == "right":
            self.shop_view.current_page = min(total - 1, self.shop_view.current_page + 1)
        elif self.action == "far_right":
            self.shop_view.current_page = total - 1
        self.shop_view._build_components()
        await interaction.response.edit_message(embed=self.shop_view.build_embed(), view=self.shop_view)


class BinderBuyButton(discord.ui.Button):
    def __init__(self, shop_view):
        super().__init__(
            label="Buy",
            style=discord.ButtonStyle.success,
            custom_id=f"bindershop_buy_{shop_view.user_id}",
            row=2
        )
        self.shop_view = shop_view
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        tid, tname, paint_price, glue_price, image_url, drop_emoji, ttype = self.shop_view.templates[self.shop_view.current_page]
        total_paint_cost = paint_price or 0
        total_glue_cost = glue_price or 0
        ...
        if user_paint < total_paint_cost or user_glue < total_glue_cost:
            price_conditions = []
            if total_paint_cost > 0: price_conditions.append(f"{PAINT_EMOJI} {total_paint_cost:,} Paint")
            if total_glue_cost > 0: price_conditions.append(f"{GLUE_EMOJI} {total_glue_cost:,} Glue")
            price_display = " & ".join(price_conditions)
            return await interaction.followup.send(
                f"❌ Not enough funds! Cost: {price_display}. You have {PAINT_EMOJI} {user_paint:,} Paint and {GLUE_EMOJI} {user_glue:,} Glue.",
                ephemeral=True
            )

        await run_query(
            "UPDATE ryo_users SET paint = paint - %s, glue = glue - %s WHERE user_id = %s",
            (total_paint_cost, total_glue_cost, buyer_id)
        )
        await run_query(
            """
            INSERT INTO ryo_binder_inventory (user_id, binder_id, quantity)
            VALUES (%s, %s, 1)
            ON CONFLICT (user_id, binder_id) DO UPDATE SET
                quantity = ryo_binder_inventory.quantity + EXCLUDED.quantity
            """,
            (buyer_id, tid)
        )

        price_parts = []
        if total_paint_cost > 0: price_parts.append(f"{total_paint_cost:,} Paint {PAINT_EMOJI}")
        if total_glue_cost > 0: price_parts.append(f"{total_glue_cost:,} Glue {GLUE_EMOJI}")
        price_str = " and ".join(price_parts) if price_parts else "Free"

        embed = discord.Embed(
            title="🎉 Binder Purchased!",
            description=f"You successfully bought **1x** `{tname}` (ID: `{tid}`) for **{price_str}**!\n"
                        f"Use `/bindercreate` to create a new personalized showcase using this theme.",
            color=RYO_COLOR
        )
...
class BinderShopView(discord.ui.View):
    def __init__(self, templates, user_id, user_avatar_url, current_page=0):
        super().__init__(timeout=300)
        # templates: list of (id, name, paint_price, glue_price, image_url, drop_emoji, type)
    ...
    def _build_components(self):
        ...
        colors = [
            (tid, tname, drop_emoji)
            for tid, tname, p, g, img, drop_emoji, ttype in self.templates
            if (ttype or "Colors") == "Colors"
        ]
        themes = [
            (tid, tname, drop_emoji)
            for tid, tname, p, g, img, drop_emoji, ttype in self.templates
            if (ttype or "Colors") == "Theme"
        ]

        row_cursor = 0
        if colors:
            self.add_item(BinderCategorySelect(colors, "Colors", self, row_num=row_cursor))
            row_cursor += 1
        if themes:
            self.add_item(BinderCategorySelect(themes, "Theme", self, row_num=row_cursor))
            row_cursor += 1

        # Buttons always on row 2 (max 2 dropdowns before them)
        at_start = self.current_page == 0
        at_end = self.current_page >= total - 1
        self.add_item(BinderBuyButton(self))
        self.add_item(BinderNavButton("<:rb_full_left:1493145347046768721>",  "far_left",  self, disabled=at_start))
        self.add_item(BinderNavButton("<:rb_left:1493145361601138721>",       "left",      self, disabled=at_start))
        self.add_item(BinderNavButton("<:rb_right:1493145374846615573>",      "right",     self, disabled=at_end))
        self.add_item(BinderNavButton("<:rb_full_right:1493145353996730421>", "far_right", self, disabled=at_end))

    def build_embed(self):
        if not self.templates:
            return discord.Embed(
                title="🛍️ Binder Shop",
                description="The binder shop is currently empty. Staff has not added any binders yet!",
                color=RYO_COLOR
            )

        total = len(self.templates)
        tid, tname, paint_price, glue_price, image_url, drop_emoji, ttype = self.templates[self.current_page]

        prices = []
        if paint_price and paint_price > 0:
            prices.append(f"{PAINT_EMOJI} {paint_price:,}")
        if glue_price and glue_price > 0:
            prices.append(f"{GLUE_EMOJI} {glue_price:,}")
        price_str = " + ".join(prices) if prices else "Free"

        embed = discord.Embed(
            description=f"# {CHALKFLOWER_EMOJI} {tname}\n\n**Purchase ID** — `{tid}`\n**Price** — {price_str}",
            color=RYO_COLOR
        )
        embed.set_author(name="binder shop 📚", icon_url=self.user_avatar_url)
        if image_url and image_url.lower().startswith("http"):
            embed.set_image(url=image_url)
        embed.set_footer(
            text=f"page {self.current_page + 1}/{total} | total items : {total} | press 'buy' below to purchase this binder"
        )
        return embed


@bot.tree.command(name="bindershop", description="View or purchase binders from the shop")
async def bindershop_cmd(interaction: discord.Interaction):
    await interaction.response.defer()

    templates = await run_query(
        "SELECT id, name, paint_price, glue_price, image_url, drop_emoji, type FROM ryo_binder_templates ORDER BY type, name",
        fetchall=True
    ) or []
    ...
    view = BinderShopView(templates, interaction.user.id, user_avatar_url)
    await interaction.followup.send(embed=view.build_embed(), view=view)

@bot.tree.command(name="binderbuy", description="Purchase a binder theme from the shop")
@app_commands.describe(
    purchase_id="The purchase ID of the binder template to buy",
    quantity="Number of copies to purchase (default 1)"
)
async def binderbuy_cmd(interaction: discord.Interaction, purchase_id: str, quantity: int = 1):
    await interaction.response.defer(ephemeral=True)
    ...
    clean_id = purchase_id.strip().lower()
    template = await run_query(
        "SELECT id, name, paint_price, glue_price FROM ryo_binder_templates WHERE id = %s",
        (clean_id,), fetchone=True
    )
    if not template:
        return await interaction.followup.send(f"❌ No binder with purchase ID `{clean_id}` exists in the shop.", ephemeral=True)

    tid, tname, paint_price, glue_price = template
    buyer_id = str(interaction.user.id)
    total_paint_cost = (paint_price or 0) * quantity
    total_glue_cost = (glue_price or 0) * quantity
    ...
    embed = discord.Embed(
        title="🎉 Binder Purchased!",
        description=f"You successfully bought **{quantity}x** `{tname}` (ID: `{tid}`) for **{price_str}**!\n"
                    f"Use `/bindercreate` to create a new personalized showcase using this theme.",
        color=RYO_COLOR
    )
    await interaction.followup.send(embed=embed, ephemeral=True)

@bot.tree.command(name="binderinv", description="View your purchased binder templates")
async def binderinv_cmd(interaction: discord.Interaction):
    await interaction.response.defer()
    uid = str(interaction.user.id)

    rows = await run_query(
        """
        SELECT i.binder_id, t.name, i.quantity
        FROM ryo_binder_inventory i
        JOIN ryo_binder_templates t ON i.binder_id = t.id
    ...
    if not rows:
        embed = discord.Embed(
            ...
            description="Your binder inventory is empty. Browse `/bindershop` and use `/binderbuy` to get some!",
            color=RYO_COLOR
        )
        return await interaction.followup.send(embed=embed)

    lines = []
    for bid, bname, qty in rows:
        qty_display = f"x{qty}" if qty > 1 else ""
        lines.append(f"📖 **{bname}** {qty_display}\nPurchase ID: `{bid}`")

    embed = discord.Embed(
        title="🎒 Your Binder Inventory",
        description="\n\n".join(lines) + "\n\nCreate active binders using `/bindercreate`!",
        color=RYO_COLOR
    )
    embed.set_footer(text="RyoBot Binder Inventory", icon_url=TEDDY_ICON_URL)
    await interaction.followup.send(embed=embed)

@bot.tree.command(name="bindercreate", description="Create a new card binder using a purchased template")
@app_commands.describe(
    purchase_id="The purchase ID of the binder template from your inventory",
    name="A unique custom name for this binder (e.g. My Favs)"
)
async def binder_create_cmd(
    interaction: discord.Interaction,
    purchase_id: str,
    name: str
):
    await interaction.response.defer(ephemeral=True)
    uid = str(interaction.user.id)
    clean_bid = purchase_id.strip().lower()
    clean_name = name.strip()
    
    if len(clean_name) < 2 or len(clean_name) > 30:
        return await interaction.followup.send("❌ Binder name must be between 2 and 30 characters.", ephemeral=True)
        
    exists = await run_query("SELECT id FROM ryo_user_binders WHERE user_id = %s AND LOWER(name) = LOWER(%s)", (uid, clean_name), fetchone=True)
    if exists:
        return await interaction.followup.send(f"❌ You already have a binder named `{clean_name}`! Please choose a different name.", ephemeral=True)
        
    inv = await run_query("SELECT quantity FROM ryo_binder_inventory WHERE user_id = %s AND binder_id = %s", (uid, clean_bid), fetchone=True)
    if not inv or inv[0] < 1:
        return await interaction.followup.send(f"❌ You do not own the binder theme `{clean_bid}` in your inventory. Buy it from `/bindershop` first!", ephemeral=True)
        
    total_binders = await run_query("SELECT COUNT(*) FROM ryo_user_binders WHERE user_id = %s", (uid,), fetchone=True)
    if total_binders and total_binders[0] >= 10:
        return await interaction.followup.send("❌ You have reached the maximum limit of 10 created binders.", ephemeral=True)
        
    await run_query("UPDATE ryo_binder_inventory SET quantity = quantity - 1 WHERE user_id = %s AND binder_id = %s", (uid, clean_bid))
    
    await run_query("INSERT INTO ryo_user_binders (user_id, binder_id, name) VALUES (%s, %s, %s)", (uid, clean_bid, clean_name))
    
    await interaction.followup.send(f"✅ Successfully created a new binder named `{clean_name}` using the template `{clean_bid}`!", ephemeral=True)

@bot.tree.command(name="binderclear", description="Clear all card slots inside a specific binder")
@app_commands.describe(binder_name="The name of the binder you want to clear")
async def binderclear_cmd(interaction: discord.Interaction, binder_name: str):
    await interaction.response.defer(ephemeral=True)
    uid = str(interaction.user.id)
    clean_name = binder_name.strip()
    
    instance = await run_query("SELECT id FROM ryo_user_binders WHERE user_id = %s AND LOWER(name) = LOWER(%s)", (uid, clean_name), fetchone=True)
    if not instance:
        return await interaction.followup.send(f"❌ You don't have a binder named `{clean_name}`.", ephemeral=True)
        
    instance_id = instance[0]
    
    await run_query("DELETE FROM ryo_binder_slots WHERE binder_instance_id::text = %s::text", (str(instance_id),))
    await interaction.followup.send(f"✅ Successfully cleared all card slots inside your binder `{clean_name}`!", ephemeral=True)

@bot.tree.command(name="binderedit", description="Edit your binder details (rename or change template theme)")
@app_commands.describe(
    binder_name="The current name of the binder to edit",
    new_name="Optional new name for this binder",
    new_binder_id="Optional new binder template ID to apply from your inventory"
)
async def binderedit_cmd(
    interaction: discord.Interaction,
    binder_name: str,
    new_name: str = None,
    new_binder_id: str = None
):
    await interaction.response.defer(ephemeral=True)
    uid = str(interaction.user.id)
    clean_name = binder_name.strip()
    
    instance = await run_query("SELECT id, binder_id FROM ryo_user_binders WHERE user_id = %s AND LOWER(name) = LOWER(%s)", (uid, clean_name), fetchone=True)
    if not instance:
        return await interaction.followup.send(f"❌ You don't have a binder named `{clean_name}`.", ephemeral=True)
        
    instance_id, current_bid = instance
    ...
    if new_name:
        ...
        if len(clean_new_name) < 2 or len(clean_new_name) > 30:
            return await interaction.followup.send("❌ New binder name must be between 2 and 30 characters.", ephemeral=True)
            
        exists = await run_query("SELECT id FROM ryo_user_binders WHERE user_id = %s AND LOWER(name) = LOWER(%s) AND id::text != %s::text", (uid, clean_new_name, str(instance_id)), fetchone=True)
        if exists:
            return await interaction.followup.send(f"❌ You already have another binder named `{clean_new_name}`!", ephemeral=True)
            
        updates.append("name = %s")
        params.append(clean_new_name)
        response_msg.append(f"renamed to `{clean_new_name}`")
        
    if new_binder_id:
        clean_new_bid = new_binder_id.strip().lower()
        if clean_new_bid != current_bid.lower():
            inv = await run_query("SELECT quantity FROM ryo_binder_inventory WHERE user_id = %s AND binder_id = %s", (uid, clean_new_bid), fetchone=True)
            if not inv or inv[0] < 1:
                return await interaction.followup.send(f"❌ You do not own the binder theme `{clean_new_bid}` in your inventory.", ephemeral=True)
            ...
            await run_query(
                ...
                VALUES (%s, %s, 1)
                ON CONFLICT (user_id, binder_id) DO UPDATE SET
                    quantity = ryo_binder_inventory.quantity + 1
                """,
                (uid, current_bid)
            )
            
            await run_query("UPDATE ryo_binder_inventory SET quantity = quantity - 1 WHERE user_id = %s AND binder_id = %s", (uid, clean_new_bid))
            
            updates.append("binder_id = %s")
            params.append(clean_new_bid)
            response_msg.append(f"theme template updated to `{clean_new_bid}`")
            
    if not updates:
        return await interaction.followup.send("⚠️ No changes specified. Please provide a new name or a new binder ID.", ephemeral=True)
        
    query = f"UPDATE ryo_user_binders SET {', '.join(updates)} WHERE id::text = %s::text"
    params.append(str(instance_id))
    await run_query(query, params)
    
    await interaction.followup.send(f"✅ Successfully updated binder `{clean_name}`: {', '.join(response_msg)}!", ephemeral=True)

@bot.tree.command(name="binderdelete", description="Delete a binder instance and reclaim the template theme back to inventory")
@app_commands.describe(binder_name="The name of the binder to delete")
async def binderdelete_cmd(interaction: discord.Interaction, binder_name: str):
    await interaction.response.defer(ephemeral=True)
    uid = str(interaction.user.id)
    clean_name = binder_name.strip()
    ...
    await run_query("DELETE FROM ryo_user_binders WHERE id::text = %s::text", (str(instance_id),))
    await run_query("DELETE FROM ryo_binder_slots WHERE binder_instance_id::text = %s::text", (str(instance_id),))
    
    await interaction.followup.send(f"✅ Successfully deleted binder `{clean_name}` and refunded the template `{current_bid}` to your inventory!", ephemeral=True)

@bot.tree.command(name="binderlist", description="List all your created binders")
async def binderlist_cmd(interaction: discord.Interaction):
    await interaction.response.defer()
    uid = str(interaction.user.id)

    binders = await run_query(
        """
        SELECT b.name, b.binder_id, t.name AS template_name
        FROM ryo_user_binders b
        LEFT JOIN ryo_binder_templates t ON b.binder_id = t.id
        WHERE b.user_id = %s
        ORDER BY b.name
    ...
    if not binders:
        embed = discord.Embed(
            title="📚 Your Binders",
            description="You haven't created any binders yet. Use `/bindercreate` to make one!",
            color=RYO_COLOR
        )
        return await interaction.followup.send(embed=embed)

    lines = []
    for i, (bname, bid, tname) in enumerate(binders, 1):
        lines.append(f"`{i}.` **{bname}** — `{bid}` ({tname or 'Unknown'})")

    embed = discord.Embed(
        title=f"📚 Your Binders ({len(binders)}/10)",
        description="\n".join(lines),
        color=RYO_COLOR
    )
    embed.set_footer(text="Use /bindershow <name> to view a binder | RyoBot Binder System", icon_url=TEDDY_ICON_URL)
    await interaction.followup.send(embed=embed)

@bot.tree.command(name="binderadd", description="Add cards to your binder slots (fills empty slots automatically)")
@app_commands.describe(
    binder_name="The name of the binder to add cards to",
    card_ids="Comma-separated list of Card IDs to add (e.g. porsche_r5, benz_r4)",
    slot="Optional specific slot (1 to 8) to place the first card into"
)
@app_commands.choices(slot=[
    app_commands.Choice(name=f"Slot {i}", value=i) for i in range(1, 9)
])
async def binder_add_cmd(
    interaction: discord.Interaction,
    binder_name: str,
    card_ids: str,
    slot: int = None
    ...
    if not raw_ids:
        return await interaction.followup.send("❌ No valid Card IDs provided.", ephemeral=True)
        
    valid_card_ids = []
    for cid in raw_ids:
        card_data = await run_query("SELECT card_id FROM ryo_cards WHERE LOWER(card_id) = LOWER(%s)", (cid,), fetchone=True)
        if card_data:
            valid_card_ids.append(card_data[0])
        else:
            return await interaction.followup.send(f"❌ Card ID `{cid}` does not exist in the registry.", ephemeral=True)
            
    owned_counts = {}
    for cid in set(valid_card_ids):
        row = await run_query("SELECT COUNT(*) FROM ryo_inventory WHERE user_id = %s AND LOWER(card_id) = LOWER(%s)", (uid, cid.lower()), fetchone=True)
        owned_counts[cid.lower()] = row[0] if row else 0
    ...
    for cid in set(valid_card_ids):
        row = await run_query(
            """
            SELECT COUNT(*) 
            FROM ryo_binder_slots s 
            JOIN ryo_user_binders b ON s.binder_instance_id::text = b.id::text 
            WHERE b.user_id = %s AND LOWER(s.card_id) = LOWER(%s)
            """,
            (uid, cid.lower()),
            fetchone=True
        )
        slotted_counts[cid.lower()] = row[0] if row else 0
        
    incoming_counts = {}
    for cid in valid_card_ids:
        incoming_counts[cid.lower()] = incoming_counts.get(cid.lower(), 0) + 1
        
    for cid_lower, incoming_qty in incoming_counts.items():
        owned_qty = owned_counts.get(cid_lower, 0)
        already_slotted = slotted_counts.get(cid_lower, 0)
        available = owned_qty - already_slotted
        
        if available < incoming_qty:
            matched_cid = next((c for c in valid_card_ids if c.lower() == cid_lower), cid_lower)
            return await interaction.followup.send(
                f"❌ Insufficient copies of `{matched_cid}`!\n"
                f"You own **{owned_qty}** copy/copies, and **{already_slotted}** are already used in your binders.\n"
                f"You are trying to add **{incoming_qty}** more.",
                ephemeral=True
            )

    filled_slots_rows = await run_query("SELECT slot, card_id FROM ryo_binder_slots WHERE binder_instance_id::text = %s::text", (str(instance_id),), fetchall=True) or []
    filled_slots = {r[0]: r[1] for r in filled_slots_rows}
    
    assigned = []
    
    if slot is not None:
        if slot < 1 or slot > 8:
            return await interaction.followup.send("❌ Slot number must be between 1 and 8.", ephemeral=True)
            
        current_target_slot = slot
        for cid in valid_card_ids:
            if current_target_slot > 8:
                break
            await run_query(
                """
                INSERT INTO ryo_binder_slots (binder_instance_id, slot, card_id)
                VALUES (%s, %s, %s)
                ON CONFLICT (binder_instance_id, slot) DO UPDATE SET
                    card_id = EXCLUDED.card_id
                """,
                (int(instance_id), current_target_slot, cid)
            )
            assigned.append((current_target_slot, cid))
            current_target_slot += 1
    else:
        empty_slots = [s for s in range(1, 9) if s not in filled_slots]
        if not empty_slots:
            return await interaction.followup.send(f"❌ Your binder `{clean_name}` is already full! Clear some slots using `/binderremove` or `/binderclear` first.", ephemeral=True)
        ...
        for cid in valid_card_ids:
            ...
            await run_query(
                ...
                ON CONFLICT (binder_instance_id, slot) DO UPDATE SET
                    card_id = EXCLUDED.card_id
                """,
                (int(instance_id), target_slot, cid)
            )
            assigned.append((target_slot, cid))
            
    if not assigned:
        return await interaction.followup.send("❌ No cards could be added (binder is full or invalid slot).", ephemeral=True)
        
    assigned_details = [f"Slot {s}: `{cid}`" for s, cid in assigned]
    await interaction.followup.send(f"✅ Successfully added cards to binder `{clean_name}`:\n" + "\n".join(assigned_details), ephemeral=True)

@bot.tree.command(name="binderremove", description="Remove cards or clear slots from your binder")
@app_commands.describe(
    binder_name="The name of the binder to remove cards from",
    card_ids="Comma-separated list of Card IDs or Slot Numbers (1 to 8) to remove"
)
async def binder_remove_cmd(
    interaction: discord.Interaction,
    binder_name: str,
    card_ids: str
):
    ...
    raw_tokens = [t.strip() for t in card_ids.split(",") if t.strip()]
    if not raw_tokens:
        return await interaction.followup.send("❌ No valid card IDs or slots provided.", ephemeral=True)
    ...
    for token in raw_tokens:
        if token.isdigit():
            val = int(token)
            if 1 <= val <= 8:
                slots_to_delete.add(val)
            else:
                return await interaction.followup.send(f"❌ Invalid slot number `{token}`. Slots must be 1 to 8.", ephemeral=True)
        else:
            cards_to_delete.add(token.lower())
    ...
    if slots_to_delete:
        ...
        res = await run_query(
            f"DELETE FROM ryo_binder_slots WHERE binder_instance_id::text = %s::text AND slot IN ({placeholders})",
            [str(instance_id)] + list(slots_to_delete),
    ...
    if cards_to_delete:
        ...
        res = await run_query(
            f"DELETE FROM ryo_binder_slots WHERE binder_instance_id::text = %s::text AND LOWER(card_id) IN ({placeholders})",
            [str(instance_id)] + list(cards_to_delete),
            return_rowcount=True
        )
        deleted_count += res
        
    if deleted_count <= 0:
        return await interaction.followup.send("❌ No matching cards or slots were found in this binder.", ephemeral=True)
        
    await interaction.followup.send(f"✅ Successfully removed **{deleted_count}** card(s) from your binder `{clean_name}`!", ephemeral=True)

@bindershow_cmd.autocomplete('binder_name')
@binder_add_cmd.autocomplete('binder_name')
@binder_remove_cmd.autocomplete('binder_name')
@binderclear_cmd.autocomplete('binder_name')
@binderedit_cmd.autocomplete('binder_name')
@binderdelete_cmd.autocomplete('binder_name')
@safe_autocomplete
async def autocomplete_binder_name(interaction: discord.Interaction, current: str):
    uid = str(interaction.user.id)
    binders = await run_query("SELECT name FROM ryo_user_binders WHERE user_id = %s", (uid,), fetchall=True) or []
    choices = []
    for b in binders:
        name = b[0]
...
@binder_create_cmd.autocomplete('purchase_id')
@binderedit_cmd.autocomplete('new_binder_id')
@safe_autocomplete
async def autocomplete_owned_binder_id(interaction: discord.Interaction, current: str):
    uid = str(interaction.user.id)
    rows = await run_query(
        """
        SELECT i.binder_id, t.name 
        FROM ryo_binder_inventory i
        JOIN ryo_binder_templates t ON i.binder_id = t.id
        WHERE i.user_id = %s AND i.quantity > 0
        """,
    ...
    for bid, bname in rows:
        label = f"{bname} ({bid})"
        if not current or current.lower() in bid.lower() or current.lower() in bname.lower():
            choices.append(app_commands.Choice(name=label[:100], value=bid))
    return choices[:25]

@binderbuy_cmd.autocomplete('purchase_id')
@delete_binder_cmd.autocomplete('id')
@safe_autocomplete
async def autocomplete_shop_binder_id(interaction: discord.Interaction, current: str):
    rows = await run_query("SELECT id, name FROM ryo_binder_templates", fetchall=True) or []
    choices = []
    for bid, bname in rows:
        label = f"{bname} ({bid})"
        if not current or current.lower() in bid.lower() or current.lower() in bname.lower():
...
async def staffrefund_cmd(
    ...
    select_query = "SELECT card_id FROM ryo_cards WHERE " + " AND ".join(conditions)
...
@bot.tree.command(name="refundremove", description="Staff: Remove cards from the refundable list")
@app_commands.describe(
    card_id="Card ID(s) to remove (comma-separated)",
    idol="Idol name(s) to remove (comma-separated)",
    group="Group name(s) to remove (comma-separated)"
)
async def refundremove_cmd(
    interaction: discord.Interaction,
    card_id: str = None,
    idol: str = None,
    group: str = None
    ...
    if idol:
        names = [n.strip().lower() for n in idol.split(",") if n.strip()]
        if names:
            # Match card IDs from ryo_cards where member_name matches
            name_rows = await run_query(
                "SELECT card_id FROM ryo_cards WHERE LOWER(member_name) IN ({})".format(", ".join(["%s"] * len(names))),
                tuple(names), fetchall=True
            ) or []
            matched_ids = [r[0].lower() for r in name_rows]
            if matched_ids:
                conditions.append("LOWER(card_id) IN ({})".format(", ".join(["%s"] * len(matched_ids))))
                params.extend(matched_ids)

    if group:
        groups = [g.strip().lower() for g in group.split(",") if g.strip()]
        if groups:
            group_rows = await run_query(
                "SELECT card_id FROM ryo_cards WHERE LOWER(group_name) IN ({})".format(", ".join(["%s"] * len(groups))),
                tuple(groups), fetchall=True
            ) or []
            matched_ids = [r[0].lower() for r in group_rows]
            if matched_ids:
                conditions.append("LOWER(card_id) IN ({})".format(", ".join(["%s"] * len(matched_ids))))
                params.extend(matched_ids)

    if not conditions:
        return await interaction.followup.send("❌ You must specify at least one filter (card_id, idol, or group).")

    delete_query = "DELETE FROM ryo_refundable_cards WHERE " + " OR ".join(conditions)
    removed = await run_query(delete_query, tuple(params), return_rowcount=True)

    await log_action(str(interaction.user.id), "refundremove", f"Removed {removed} cards from refundable list.")
    await interaction.followup.send(f"✅ Removed **{removed}** card(s) from the refundable list.")

class RefundListView(discord.ui.View):
    def __init__(self, pages, total_rows):
    ...
    def make_embed(self):
        lines = []
        for card_id, member, group, era, rarity, category in self.pages[self.page]:
            stars = get_rarity_display(rarity, category, era, card_id)
            name_part = f"{group or '?'} {member or '?'}".strip()
            era_part = f"({era})" if era else ""
            lines.append(f"`{card_id}` — **{name_part}** {era_part} {stars}")
        embed = discord.Embed(
            title="📋 Refundable Cards List",
            description="\n".join(lines) or "—",
            color=RYO_COLOR
        )
        embed.set_footer(text=f"Page {self.page + 1}/{self.total_pages} • Total: {self.total_rows} card(s)")
        return embed
...
@bot.tree.command(name="refundlist", description="List all cards currently in the refundable list (STAFF/MOD ONLY)")
async def refundlist_cmd(interaction: discord.Interaction):
    if not (is_staff(interaction) or is_mod(interaction)):
        return await interaction.response.send_message("❌ Permission denied. Only Staff/Mods can use this command.", ephemeral=True)

    await interaction.response.defer(ephemeral=True)

    rows = await run_query(
        """
        SELECT rc.card_id, c.member_name, c.group_name, c.era, c.rarity, c.category
        FROM ryo_refundable_cards rc
        LEFT JOIN ryo_cards c ON LOWER(rc.card_id) = LOWER(c.card_id)
        ORDER BY rc.card_id
        """,
        fetchall=True
    ) or []
    ...
    if not rows:
        return await interaction.followup.send("📋 The refundable list is currently empty.")

    PAGE_SIZE = 10
    pages = [rows[i:i + PAGE_SIZE] for i in range(0, len(rows), PAGE_SIZE)]

    view = RefundListView(pages, len(rows))
    await interaction.followup.send(embed=view.make_embed(), view=view if len(pages) > 1 else None)
...
async def refund_cmd(
    ...
    # Verify card existences, rarities and categories
    all_checked_ids = list(set(exchange_ids + to_ids))
    card_rows = await run_query("SELECT card_id, rarity, category FROM ryo_cards WHERE UPPER(card_id) IN ({})".format(", ".join(["%s"] * len(all_checked_ids))), all_checked_ids, fetchall=True) or []
    rarity_map = {row[0].upper(): row[1] for row in card_rows}
    category_map = {row[0].upper(): (row[2] or "regular").lower().strip() for row in card_rows}
    ...
    # Match rarities and categories exactly in order
    for idx, (exc_id, to_id) in enumerate(zip(exchange_ids, to_ids)):
        ...
        exc_cat = category_map[exc_id]
        to_cat = category_map[to_id]
        is_exc_regular = exc_cat in ("", "regular", "custom")
        is_to_regular = to_cat in ("", "regular", "custom")
        if is_exc_regular != is_to_regular:
            return await interaction.followup.send(f"❌ Mismatch at position {idx+1}: You cannot exchange between regular and event cards! `{exc_id}` is a {'regular' if is_exc_regular else 'special event'} card, but target `{to_id}` is a {'regular' if is_to_regular else 'special event'} card.")
...
@app_commands.describe(
    ...
    card_ids="Comma separated Card IDs (e.g. porsche_r5, benz_r4)",
    copies="Number of copies for each card (defaults to 1)"
)
async def staffgift(
    interaction: discord.Interaction, 
    user: discord.Member,
    ...
    # Split card IDs by comma and clean up whitespace
    raw_ids = [c.strip() for c in card_ids.split(",") if c.strip()]
    ...
    async def on_timeout(self):
        if self.message:
            ...
            if self.message.embeds:
                ...
                embed.title = "⏰ Time's Up! (20 seconds)"
                embed.description = f"The correct {self.mode} was **{self.correct_answer}**.\nYou had 20 seconds — try to be faster next time!"
    ...
    def make_callback(self, choice):
        async def callback(interaction: discord.Interaction):
            ...
            if selected == actual:
                ...
                if not on_cooldown:
                    await run_query("UPDATE ryo_users SET last_guess_streak = guess_streak, guess_streak = 0 WHERE user_id = %s", (uid,))
                else:
...
@bot.tree.command(name="streakrestore", description="Recover your guess streak after losing one — costs 30,000 Paint")
async def streakrestore_cmd(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    uid = str(interaction.user.id)

    await run_query("INSERT INTO ryo_users (user_id) VALUES (%s) ON CONFLICT (user_id) DO NOTHING", (uid,))
    user_data = await run_query("SELECT guess_streak, last_guess_streak, paint FROM ryo_users WHERE user_id = %s", (uid,), fetchone=True)
    if not user_data:
        return await interaction.followup.send("❌ Could not retrieve your profile. Please try again.")

    current_streak, last_streak, paint = user_data
    last_streak = last_streak or 0

    if current_streak > 0:
        return await interaction.followup.send(f"❌ Your streak is currently **{current_streak}** — you don't need to restore it!")

    if last_streak <= 0:
        return await interaction.followup.send("❌ There is no previous streak on record to restore.")

    RESTORE_COST = 30000
    if paint < RESTORE_COST:
        return await interaction.followup.send(
            f"❌ You need **30,000** {PAINT_EMOJI} Paint to restore your streak, but you only have **{paint:,}**."
        )

    await run_query(
        "UPDATE ryo_users SET paint = paint - %s, guess_streak = %s, last_guess_streak = 0 WHERE user_id = %s",
        (RESTORE_COST, last_streak, uid)
    )

    embed = discord.Embed(
        title="🔥 Streak Restored!",
        description=(
            f"You spent **30,000** {PAINT_EMOJI} Paint to restore your guess streak.\n\n"
            f"🔥 Streak restored to: **{last_streak}**"
        ),
        color=RYO_COLOR
    )
    embed.set_footer(text="Keep the streak alive!", icon_url=TEDDY_ICON_URL)
    await interaction.followup.send(embed=embed)

@bot.tree.command(name="setstreak", description="Set a user's guess streak to a specific value (STAFF/MOD ONLY)")
@app_commands.describe(
    user="The user whose streak you want to set",
    streak="The streak value to assign"
)
async def setstreak_cmd(interaction: discord.Interaction, user: discord.Member, streak: int):
    if not (is_staff(interaction) or is_mod(interaction)):
        return await interaction.response.send_message("❌ Permission denied. Only Staff/Mods can use this command.", ephemeral=True)

    if streak < 0:
        return await interaction.response.send_message("❌ Streak value must be 0 or greater.", ephemeral=True)
    ...
    await run_query("INSERT INTO ryo_users (user_id) VALUES (%s) ON CONFLICT (user_id) DO NOTHING", (uid,))
    await run_query(
        "UPDATE ryo_users SET guess_streak = %s WHERE user_id = %s",
        (streak, uid)
    )

    embed = discord.Embed(
        title="✅ Streak Updated",
        description=f"{user.mention}'s guess streak has been set to **{streak}**.",
        color=RYO_COLOR
    )
    embed.set_footer(text=f"Set by {interaction.user.name}", icon_url=interaction.user.display_avatar.url)
    await interaction.followup.send(embed=embed)
...
class BurnConfirm(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if inventory_ids:
            ...
            query = "DELETE FROM ryo_inventory WHERE id::text IN %s"
            await run_query(query, (tuple(str(x) for x in inventory_ids),))
...
class GiftConfirm(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if inventory_ids:
            # Transfer cards
            query = "UPDATE ryo_inventory SET user_id = %s WHERE id::text IN %s"
            await run_query(query, (str(self.recipient.id), tuple(str(x) for x in inventory_ids)))
...
async def collection_cmd(
    ...
    # 1. Build Query
    query = "SELECT card_id, member_name, era, group_name, rarity, image_url, category FROM ryo_cards WHERE 1=1 AND COALESCE(LOWER(category), '') != 'guess_minigame' AND COALESCE(LOWER(category), '') != 'custom' AND COALESCE(LOWER(era), '') != 'guess'"
    params = []
    filter_labels = []
...
async def search(
    ...
    # Query all cards
    q = "SELECT card_id, member_name, era, group_name, rarity, image_url, category FROM ryo_cards WHERE COALESCE(LOWER(category), '') != 'guess_minigame' AND COALESCE(LOWER(category), '') != 'custom' AND COALESCE(LOWER(era), '') != 'guess' ORDER BY rarity DESC, member_name ASC"
...
class MarketView(discord.ui.View):
    def __init__(self, title, listings, user_id=None, filters=None, user=None):
        super().__init__(timeout=120)
        self.title = title
    ...
    def create_embed(self):
        ...
        embed = discord.Embed(color=RYO_COLOR)
        if self.user:
            embed.set_author(name=f"{self.user.name} is viewing the market!", icon_url=self.user.display_avatar.url)
        else:
            embed.title = self.title
        ...
        for item in current_listings:
            ...
            line_1 = f"x1 {stars} {name_line} ({era})"
            line_2 = f"`{market_id}` {paint_price} 🎨 • {glue_price} 🧪 Sold by <@{seller_id}>"
...
async def market_sell(interaction: discord.Interaction, codes: str, paint_price: int = 0, glue_price: int = 0):
    ...
    for code_lower, count in needed_counts.items():
        for _ in range(count):
            ...
            await run_query("DELETE FROM ryo_inventory WHERE id::text = %s::text", (str(inv_id),))
...
async def market_view(
    ...
    view = MarketView("🛒 Card Marketplace", filtered_listings, user_id=viewer_id, filters=filters_summary, user=interaction.user)
    await interaction.followup.send(embed=view.create_embed(), view=view)
...
@app_commands.describe(
    id="The unique number ID of the pack (e.g. 1, 2, 3)",
    name="The display name of the pack",
    description="The details of the pack",
    paint_price="Price in Paint (0 for none)",
    glue_price="Price in Glue (0 for none)",
    cooldown="Cooldown in hours",
    no_of_cards="Number of cards inside",
    rarities="Rarities (comma separated, e.g., 3,4,5)",
    era_filter="Optional Era filter (leave blank for none)"
)
async def shop_add(
    interaction: discord.Interaction,
    id: int,
    ...
    if paint_price < 0 or glue_price < 0:
        return await interaction.response.send_message("❌ Prices cannot be negative.", ephemeral=True)
        
    if cooldown < 0 or no_of_cards <= 0:
        return await interaction.response.send_message("❌ Cooldown cannot be negative and number of cards must be greater than 0.", ephemeral=True)

    pack_id_clean = str(id)
    name_clean = name.strip()
    desc_clean = description.strip()
    
    if not name_clean or not desc_clean:
         return await interaction.response.send_message("❌ Name and description cannot be empty.", ephemeral=True)
         
    # Validate rarities format
    rarity_parts = [r.strip() for r in rarities.split(",") if r.strip()]
    for r in rarity_parts:
        if not r.isdigit():
            return await interaction.response.send_message(f"❌ Rarity `{r}` is not a valid number.", ephemeral=True)
            
    # Check if exists to choose message wording
    exists = await run_query("SELECT 1 FROM ryo_shop_packs WHERE pack_id = %s", (pack_id_clean,), fetchone=True)
    
    await run_query(
        """
        INSERT INTO ryo_shop_packs (pack_id, name, description, price, glue_price, cooldown, no_of_cards, rarity, era)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
        ON CONFLICT (pack_id) DO UPDATE SET
            ...
            era = EXCLUDED.era
        """,
        (pack_id_clean, name_clean, desc_clean, paint_price, glue_price, cooldown, no_of_cards, ",".join(rarity_parts), era_filter.strip() if era_filter else None)
    )
...
async def shop_view(interaction: discord.Interaction):
    packs = await run_query(
        "SELECT pack_id, name, description, price, glue_price FROM ryo_shop_packs ORDER BY CASE WHEN pack_id ~ '^[0-9]+$' THEN pack_id::bigint ELSE 999999 END ASC, pack_id ASC",
        fetchall=True
    )
    if not packs:
        return await interaction.response.send_message(
            "\U0001f3ea The Ryo Pack Shop is currently empty. Staff have not listed any packs yet!", 
            ephemeral=True
        )
        
    desc_lines = []
    for p in packs:
        pack_id, name, description, price, glue_price = p
        
        # Display prices
        prices_str = []
        if price > 0:
            prices_str.append(f"{PAINT_EMOJI} {price:,} Paint")
        if glue_price > 0:
            prices_str.append(f"{GLUE_EMOJI} {glue_price:,} Glue")
            
        prices_txt = " / ".join(prices_str) if prices_str else "Free"
...
@bot.tree.command(name="shopbuy", description="Purchase a card pack from the shop using Paint/Glue")
@app_commands.describe(
...
async def shop_buy(interaction: discord.Interaction, id: str, quantity: int, group: str = None):
    ...
    pack_id_clean = id.strip()
    pack = await run_query("SELECT pack_id, name, description, price, glue_price, cooldown, no_of_cards, rarity, era FROM ryo_shop_packs WHERE LOWER(pack_id) = LOWER(%s)", (pack_id_clean,), fetchone=True)
    if not pack:
        return await interaction.followup.send(f"❌ Pack with ID `{pack_id_clean}` does not exist in the shop registry.")
        
    real_pack_id, pack_name, description, paint_price, glue_price, cooldown_val, no_of_cards, rarities_str, era_filter = pack
    
    # Ensure user exists
    await run_query("INSERT INTO ryo_users (user_id) VALUES (%s) ON CONFLICT (user_id) DO NOTHING", (buyer_id,))
    
    # Check cooldown
    if cooldown_val and cooldown_val > 0:
        rem_time = await check_pack_cooldown(buyer_id, real_pack_id, cooldown_val * 3600)
        if rem_time > 0:
            target_time = int(time.time() + rem_time)
            return await interaction.followup.send(f"❌ This pack is on cooldown! You can purchase it again <t:{target_time}:R>.")
    ...
    if user_paint < total_paint_cost or user_glue < total_glue_cost:
        ...
        if total_glue_cost > 0: price_conditions.append(f"{GLUE_EMOJI} {total_glue_cost:,} Glue")
        price_display = " & ".join(price_conditions)
        return await interaction.followup.send(f"❌ You do not have enough funds! Price is {price_display}. You currently have {PAINT_EMOJI} {user_paint:,} Paint and {GLUE_EMOJI} {user_glue:,} Glue.")
    ...
    # Check if a custom rarity option was explicitly configured/mentioned (i.e. not the default "1,2,3,4,5")
    has_custom_rarity = rarities_str and rarities_str.strip() != "1,2,3,4,5"
    ...
    if "group" in pack_id_lower or "5g" in pack_id_lower or "group" in name_lower or "group" in desc_lower or "guaranteed" in desc_lower:
        is_group_pack = True
    elif not has_custom_rarity and ("r5" in pack_id_lower or "5star" in pack_id_lower or "r5" in name_lower or "r5" in desc_lower or "5-star" in desc_lower or "5 star" in desc_lower):
        is_r5_pack = True
    ...
    if is_group_pack:
        ...
        # Dynamic pull logic based on pack's configured options (PangPond style)
        rarity_list = [int(r.strip()) for r in (rarities_str or "1,2,3,4,5").split(",") if r.strip().isdigit()]
        query_str = "SELECT card_id, member_name, group_name, era, rarity, image_url, category FROM ryo_cards WHERE LOWER(category) != 'guess_minigame' AND LOWER(era) != 'guess' AND LOWER(category) != 'custom' AND (disabled IS NULL OR disabled = FALSE)"
        query_params = []
        
        # Filter by allowed rarities
        if rarity_list:
            query_str += " AND rarity IN ({})".format(", ".join(["%s"] * len(rarity_list)))
            query_params.extend(rarity_list)
            
        # Filter by era
        if era_filter:
            query_str += " AND LOWER(era) = LOWER(%s)"
            query_params.append(era_filter)
            
        pool_cards = await run_query(query_str, query_params, fetchall=True) or []
        if not pool_cards:
            # Fallback if no cards match filters
            pool_cards = await run_query("SELECT card_id, member_name, group_name, era, rarity, image_url, category FROM ryo_cards WHERE LOWER(category) != 'guess_minigame' AND LOWER(era) != 'guess' AND LOWER(category) != 'custom' AND (disabled IS NULL OR disabled = FALSE)", fetchall=True) or []
            
        if not pool_cards:
            return await interaction.followup.send("❌ Error: No cards found in the registry to draw.")
            
        num_cards_per_pack = no_of_cards or 3
        drawn_cards = random.choices(pool_cards, k=num_cards_per_pack * quantity)

    # Perform currency subtraction
    await run_query("UPDATE ryo_users SET paint = paint - %s, glue = glue - %s WHERE user_id = %s", (total_paint_cost, total_glue_cost, buyer_id))
    
    # Set cooldown if applicable
    if cooldown_val and cooldown_val > 0:
        await update_pack_cooldown(buyer_id, real_pack_id)
    ...
    embed = discord.Embed(
        ...
        description=f"<@{buyer_id}> bought **{quantity}x** `{pack_name}` for **{price_str}**!\n\n"
...
async def gift(
    ...
    for item in inventory:
        ...
        # Block custom cards from regular user gifting
        if item[3] == 'custom': continue
