...
async def grant_card(user_id: str, card_id: str):
    ...
    is_guess = await run_query(
        "SELECT 1 FROM ryo_cards WHERE card_id = %s AND (category = 'guess_minigame' OR era = 'Guess')",
    ...
    await run_query(
        ...
        (str(user_id), card_id, code)
    )
    return code

GUILD_IDS = [
    1424877893514952776, # Main Ryo Guild
    1475099422315647006, # Pangpond Priority Guild 1
    1493865839961706517  # Pangpond Priority Guild 2
]
...
async def update_autocomplete_cache(force=False):
    ...
    try:
        # Fetch data excluding guess minigame
        groups_task = run_query("SELECT DISTINCT group_name FROM ryo_cards WHERE group_name IS NOT NULL AND era != 'Guess' AND category != 'guess_minigame'", fetchall=True)
        eras_task = run_query("SELECT DISTINCT era FROM ryo_cards WHERE era IS NOT NULL AND era != 'Guess' AND category != 'guess_minigame'", fetchall=True)
        names_task = run_query("SELECT DISTINCT member_name FROM ryo_cards WHERE member_name IS NOT NULL AND era != 'Guess' AND category != 'guess_minigame'", fetchall=True)
...
async def run_query(query, params=None, fetchone=False, fetchall=False, retries=1):
    if not db_pool:
        return None
...
async def setup_database():
    ...
    queries = [
        ...
        "INSERT INTO ryo_quests (name, description, quest_type, target_value, reward_paint, reward_glue, is_daily, category) VALUES ('Daily Drawer', 'Draw 10 cards today', 'draw', 10, 50, 20, TRUE, 'Daily') ON CONFLICT DO NOTHING;",
        "INSERT INTO ryo_quests (name, description, quest_type, target_value, reward_paint, reward_glue, is_daily, category) VALUES ('Generous Soul', 'Gift 3 cards to friends', 'gift', 3, 30, 10, TRUE, 'Daily') ON CONFLICT DO NOTHING;",
        "INSERT INTO ryo_quests (name, description, quest_type, target_value, reward_paint, reward_glue, is_daily, category) VALUES ('Card Burner', 'Burn 5 unwanted cards', 'burn', 5, 20, 50, TRUE, 'Daily') ON CONFLICT DO NOTHING;",
    ...
    for cmd in ["draw", "paint", "daily", "weekly", "event-draw", "guess-reward", "sticky", "color"]:
...
class RyoBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        try:
            # 🧹 CLEAN UP: Clear guild-level commands to prevent duplicates with global ones
            # Discord shows 2 of each command if both global and guild-specific syncs were used.
            # We clear the guild level for all priority guilds to ensure only the global ones remain.
            for g_id in GUILD_IDS:
                try:
                    guild = discord.Object(id=g_id)
                    self.tree.clear_commands(guild=guild)
                    await self.tree.sync(guild=guild)
                    print(f"🧹 Cleared guild commands for {g_id}")
                except Exception as e:
                    print(f"⚠️ Could not clear guild commands for {g_id}: {e}")
...
async def daily(interaction: discord.Interaction, reminder: bool = None):
    ...
    if now - last_used < 86400:
        ...
        return await interaction.followup.send(f"⌛ Ryo is preparing your gifts! Try again in {int(hours)}h {int(minutes)}m.", ephemeral=True)
...
async def weekly(interaction: discord.Interaction, reminder: bool = None):
    ...
    if now - last_used < 604800:
        ...
        return await interaction.followup.send(f"⌛ Ryo is saving up your weekly stash! Try again in {int(days)}d {int(hours)}h.", ephemeral=True)
...
async def sticky(interaction: discord.Interaction, reminder: bool = None):
    ...
    if now - last_used < 3600:
        ...
        return await interaction.followup.send(f"⌛ Ryo is still cleaning up! Try again in {rem//60}m {rem%60}s.", ephemeral=True)
...
async def color(interaction: discord.Interaction, reminder: bool = None):
    ...
    if now - last_used < 1800:
        ...
        return await interaction.followup.send(f"⌛ Ryo is still mixing the paints! Try again in {rem//60}m {rem%60}s.", ephemeral=True)
...
class DrawChoiceView(discord.ui.View):
    ...
    async def on_timeout(self):
        ...
        if self.message:
            try: 
                # Edit original message to disable interaction
                await self.message.edit(view=self)
                # Reply to original message as requested
...
async def draw_cmd(interaction: discord.Interaction, reminder: bool = None):
    ...
    if now - last_used < cd_seconds:
        ...
        return await interaction.followup.send(f"⌛ Ryo is dealing your next hand! Try again in {rem//60}m {rem%60}s.", ephemeral=True)
...
async def paint_drop(interaction: discord.Interaction, reminder: bool = None):
    ...
    if now - last_used < 1200:
        ...
        return await interaction.followup.send(f"⌛ Ryo is drying the mystery cards! Try again in {rem//60}m {rem%60}s.", ephemeral=True)
...
async def bulkeditcard(
    ...
    try:
        ...
        query_select = f"SELECT card_id FROM ryo_cards WHERE card_id IN ({placeholders})"
        existing_rows = await run_query(query_select, tuple(id_list), fetchall=True) or []
...
class AddGuessModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # interaction, card_id, member, era, group, category, image_url, image_file, rarity, ephemeral
        if self.mode == "Idol":
            await addcard_logic(interaction, card_id, name, "Guess", "Solo", "guess_minigame", url, None, 1, True)
        else:
            await addcard_logic(interaction, card_id, "Group", "Guess", name, "guess_minigame", url, None, 1, True)
...
class RemoveGuessModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if cid:
            ...
            await interaction.followup.send(f"🗑️ Removed entry number `{val}`.")
...
@bot.tree.command(name="addgti", description="Add a new Guess The Idol card (STAFF/MOD ONLY)")
@app_commands.describe(name="Idol Name", image_url="Direct image link", image_file="Upload an image file")
async def addgti(interaction: discord.Interaction, name: str, image_url: str = None, image_file: discord.Attachment = None):
    if not (is_staff(interaction) or is_mod(interaction)):
        return await interaction.response.send_message("❌ Unauthorized", ephemeral=True)
    ...
    await addcard_logic(interaction, card_id, name, "Guess", "Solo", "guess_minigame", image_url, image_file, 1, True)

@bot.tree.command(name="addgtg", description="Add a new Guess The Group card (STAFF/MOD ONLY)")
@app_commands.describe(name="Group Name", image_url="Direct image link", image_file="Upload an image file")
async def addgtg(interaction: discord.Interaction, name: str, image_url: str = None, image_file: discord.Attachment = None):
    if not (is_staff(interaction) or is_mod(interaction)):
        return await interaction.response.send_message("❌ Unauthorized", ephemeral=True)
    ...
    await addcard_logic(interaction, card_id, "Group", "Guess", name, "guess_minigame", image_url, image_file, 1, True)
...
async def staffgift(
    ...
    for c_id, count in potential_gifts:
        if c_id:
            # Check if card exists
            card_data = await run_query("SELECT member_name, rarity, group_name, era FROM ryo_cards WHERE card_id = %s", (c_id,), fetchone=True)
            if card_data:
                valid_gifts.append({
                    "id": c_id,
                    "name": card_data[0],
    ...
    for c_id, count in potential_removals:
        if c_id:
            ...
            if card_data:
                ...
                count_res = await run_query("SELECT COUNT(*) FROM ryo_inventory WHERE user_id = %s AND card_id = %s", (str(user.id), c_id), fetchone=True)
...
class InventoryView(discord.ui.View):
    def __init__(self, title, cards, timeout=60, user=None, filters=None, author_name=None):
    ...
    def create_embed(self):
        ...
        card_list = []
        for i, card in enumerate(current_cards, start + 1):
            if len(card) >= 5:
                ...
                # Updated: stars (era) instead of (stars) era
                line = f"{i}. {name_line}\n{stars} ({era})\n`{cid}`"
...
async def inventory(
    ...
    view = InventoryView(f"{target_user.name}'s Inventory", filtered, user=target_user, filters=filters, author_name=f"{target_user.name}'s inventory!")
...
class QuestsView(discord.ui.View):
    ...
    async def show_quests(self, interaction: discord.Interaction, category: str):
        ...
        for i, (q_id, name, desc, target, r_paint, r_glue, is_daily) in enumerate(all_quests, 1):
            ...
            if prog:
                ...
                if is_daily:
                    ...
                    if current_day > last_day:
                        current_val, is_completed, last_reset = (0, False, now_ms)
                        db_ops.append(run_query("UPDATE user_quests SET current_value = 0, completed = FALSE, last_reset = %s WHERE user_id = %s AND quest_id = %s", (last_reset, uid, q_id)))
            else:
                last_reset = now_ms
                db_ops.append(run_query("INSERT INTO user_quests (user_id, quest_id, last_reset) VALUES (%s, %s, %s)", (uid, q_id, last_reset)))
        ...
        for num, q_id, name, desc, val, target, completed, r_paint, r_glue in quest_results:
        ...
        embed.set_footer(text="Daily quests reset at midnight UTC.")
        await interaction.edit_original_response(embed=embed, view=self)
...
async def view(interaction: discord.Interaction, card_id: str):
    ...
    card = await run_query(
        "SELECT card_id, member_name, era, group_name, rarity, image_url, category FROM ryo_cards WHERE card_id = %s",
        (card_id,),
        fetchone=True
    )
...
@app_commands.describe(reminder="Turn reminder on/off")
async def event_draw(interaction: discord.Interaction, reminder: bool = None):
    await interaction.response.defer()
    uid = str(interaction.user.id)
    ...
    if now - last_used < 3600:
        rem = 3600 - (now - last_used)
        return await interaction.followup.send(f"⌛ Ryo is preparing the event stage! Try again in {rem//3600}h {(rem%3600)//60}m {rem % 60}s.", ephemeral=True)
    ...
    try:
        if reminder is not None:
            await run_query("INSERT INTO ryo_reminder_settings (user_id, command, enabled) VALUES (%s, 'event-draw', %s) ON CONFLICT (user_id, command) DO UPDATE SET enabled=%s", (uid, reminder, reminder))
        
        rem_enabled = await run_query("SELECT enabled FROM ryo_reminder_settings WHERE user_id=%s AND command='event-draw'", (uid,), fetchone=True)
        if not rem_enabled or rem_enabled[0]:
            await set_reminder(uid, "event-draw", 3600, interaction.channel_id)
...
async def startevent(interaction: discord.Interaction, category: str, era: str):
    ...
    await interaction.followup.send(f"✅ Event started! Cards for `{category}` in `{era}` are now droppable.")
...
async def missing_images(
    ...
    if group:
        query += " AND group_name ILIKE %s"
        params.append(f"%{group}%")
        
    if era:
        query += " AND era ILIKE %s"
        params.append(f"%{era}%")
        
    if name:
        query += " AND member_name ILIKE %s"
...
class AddStaffBlockModal(discord.ui.Modal):
    def __init__(self, parent_view):
        super().__init__(title="Add Approved Blockable")
        self.parent_view = parent_view
        
    category = discord.ui.TextInput(
        label="Type ('group' or 'idol')",
        placeholder="group or idol",
        max_length=15,
        required=True
    )
    ...
    name = discord.ui.TextInput(
        label="Name",
        placeholder="Exact name of Group or Idol",
        max_length=100,
        required=True
    )

    async def on_submit(self, interaction: discord.Interaction):
        if interaction.user.id not in [691771271129858129, DEV_ID]:
            return await interaction.response.send_message("❌ Only the Owner and developers can add/remove blocked groups/idols.", ephemeral=True)
            
        await interaction.response.defer(ephemeral=True)
        
        cat = self.category.value.strip().lower()
        if cat not in ['group', 'idol']:
            return await interaction.followup.send("❌ Category MUST be either 'group' or 'idol'!", ephemeral=True)
            
        val_name = self.name.value.strip()
        
        column = "group_name" if cat == "group" else "member_name"
        canonical_res = await run_query(f"SELECT DISTINCT {column} FROM ryo_cards WHERE {column} ILIKE %s LIMIT 1", (val_name,), fetchone=True)
        if not canonical_res:
            return await interaction.followup.send(f"❌ `{val_name}` was not found in any card as a {cat} in the registry! Check spelling/typos.", ephemeral=True)
            
        canonical_name = canonical_res[0]
        
        exists = await run_query("SELECT 1 FROM ryo_staff_blocks WHERE entity_type = %s AND entity_name = %s", (cat, canonical_name), fetchone=True)
        if exists:
            return await interaction.followup.send(f"⚠️ `{canonical_name}` is already in the approved blockable list.", ephemeral=True)
            
        await run_query("INSERT INTO ryo_staff_blocks (entity_type, entity_name) VALUES (%s, %s)", (cat, canonical_name))
        
        new_blocks = await run_query("SELECT entity_type, entity_name FROM ryo_staff_blocks ORDER BY entity_type ASC, entity_name ASC", fetchall=True) or []
        self.parent_view.blocks = new_blocks
        self.parent_view.total_pages = (len(new_blocks) - 1) // self.parent_view.per_page + 1 if new_blocks else 1
        
        await interaction.followup.send(f"✅ Added `{canonical_name}` as a blockable `{cat}` successfully!", ephemeral=True)
        
        if interaction.message:
            try:
                await interaction.message.edit(embed=self.parent_view.create_embed(), view=self.parent_view)
            except Exception as e:
                print(f"Failed to edit message in AddStaffBlockModal: {e}")


class RemoveStaffBlockModal(discord.ui.Modal):
    def __init__(self, parent_view):
        super().__init__(title="Remove Approved Blockable")
        self.parent_view = parent_view
    ...
    name = discord.ui.TextInput(
        label="Name",
        placeholder="Spelling of Group or Idol in list",
        max_length=100,
        required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        exists = await run_query("SELECT entity_name FROM ryo_staff_blocks WHERE entity_type = %s AND entity_name ILIKE %s LIMIT 1", (cat, val_name), fetchone=True)
        if not exists:
            return await interaction.followup.send(f"❌ `{val_name}` not found in approved block list for `{cat}`.", ephemeral=True)
            
        exact_name = exists[0]
        
        await run_query("DELETE FROM ryo_staff_blocks WHERE entity_type = %s AND entity_name = %s", (cat, exact_name))
        await run_query("DELETE FROM ryo_blocks WHERE entity_type = %s AND entity_name = %s", (cat, exact_name))
        ...
        await interaction.followup.send(f"✅ Removed `{exact_name}` from approved blockable list and player blocks.", ephemeral=True)
        
        if interaction.message:
            try:
                await interaction.message.edit(embed=self.parent_view.create_embed(), view=self.parent_view)
            except Exception as e:
                print(f"Failed to edit message in RemoveStaffBlockModal: {e}")


class StaffBlockView(discord.ui.View):
    def __init__(self, blocks, user_id, timeout=120):
        super().__init__(timeout=timeout)
        self.blocks = blocks
        self.user_id = user_id
        self.page = 0
        self.per_page = 15
        self.total_pages = (len(blocks) - 1) // self.per_page + 1 if blocks else 1

    def create_embed(self):
        start = self.page * self.per_page
        end = start + self.per_page
        current_blocks = self.blocks[start:end]
        
        embed = discord.Embed(
            title="🚫 Master Blockable List (Staff Control)",
            description="These are the groups/idols that are eligible/approved for players to block.",
            color=RYO_COLOR
        )
        
        lines = []
        for i, (btype, bname) in enumerate(current_blocks):
            lines.append(f"**{start + i + 1}.** [{btype.upper()}] **{bname}**")
            
        list_str = "\n".join(lines)
        if not list_str:
            list_str = "*No staff blocks defined yet.*"
            
        embed.description += f"\n\n{list_str}"
        embed.set_footer(text=f"Page {self.page + 1} of {self.total_pages} | Total: {len(self.blocks)}")
        return embed

    @discord.ui.button(emoji="<:rb_full_left:1493145347046768721>", style=discord.ButtonStyle.secondary, row=0)
    async def first_page(self, interaction: discord.Interaction, button: discord.ui.Button):
        if str(interaction.user.id) != self.user_id:
            return await interaction.response.send_message("❌ This is not your menu!", ephemeral=True)
        self.page = 0
    ...
    @discord.ui.button(emoji="<:rb_left:1493145361601138721>", style=discord.ButtonStyle.secondary, row=0)
    async def prev_page(self, interaction: discord.Interaction, button: discord.ui.Button):
        if str(interaction.user.id) != self.user_id:
            return await interaction.response.send_message("❌ This is not your menu!", ephemeral=True)
        if self.page > 0:
    ...
    @discord.ui.button(emoji="<:rb_right:1493145374846615573>", style=discord.ButtonStyle.secondary, row=0)
    async def next_page(self, interaction: discord.Interaction, button: discord.ui.Button):
        if str(interaction.user.id) != self.user_id:
            return await interaction.response.send_message("❌ This is not your menu!", ephemeral=True)
        if self.page < self.total_pages - 1:
    ...
    @discord.ui.button(emoji="<:rb_full_right:1493145353996730421>", style=discord.ButtonStyle.secondary, row=0)
    async def last_page(self, interaction: discord.Interaction, button: discord.ui.Button):
        if str(interaction.user.id) != self.user_id:
            return await interaction.response.send_message("❌ This is not your menu!", ephemeral=True)
        self.page = self.total_pages - 1
    ...
    @discord.ui.button(label="Add Blockable", style=discord.ButtonStyle.success, row=1)
    async def add_blockable(self, interaction: discord.Interaction, button: discord.ui.Button):
        if str(interaction.user.id) != self.user_id:
            return await interaction.response.send_message("❌ This is not your menu!", ephemeral=True)
        if interaction.user.id not in [691771271129858129, DEV_ID]:
            return await interaction.response.send_message("❌ Only the Owner and developers can add/remove blocked groups/idols.", ephemeral=True)
        
        modal = AddStaffBlockModal(self)
        await interaction.response.send_modal(modal)

    @discord.ui.button(label="Remove Blockable", style=discord.ButtonStyle.danger, row=1)
    async def remove_blockable(self, interaction: discord.Interaction, button: discord.ui.Button):
        if str(interaction.user.id) != self.user_id:
            return await interaction.response.send_message("❌ This is not your menu!", ephemeral=True)
        if interaction.user.id not in [691771271129858129, DEV_ID]:
            return await interaction.response.send_message("❌ Only the Owner and developers can add/remove blocked groups/idols.", ephemeral=True)
        
        modal = RemoveStaffBlockModal(self)
        await interaction.response.send_modal(modal)


@bot.tree.command(name="staffblock", description="Manage the allowed block list for players (STAFF/MOD ONLY)")
async def staffblock(interaction: discord.Interaction):
    if not (is_staff(interaction) or is_mod(interaction) or interaction.user.id in [691771271129858129, DEV_ID]):
        return await interaction.response.send_message("❌ Unauthorized", ephemeral=True)
        
    await interaction.response.defer(ephemeral=True)
    
    blocks = await run_query("SELECT entity_type, entity_name FROM ryo_staff_blocks ORDER BY entity_type ASC, entity_name ASC", fetchall=True) or []
    
    view = StaffBlockView(blocks, str(interaction.user.id))
    embed = view.create_embed()
...
async def card_id_autocomplete(interaction: discord.Interaction, current: str):
    ...
    try:
        ...
        results = await run_query(
            "SELECT card_id, member_name, era FROM ryo_cards WHERE (card_id ILIKE %s OR member_name ILIKE %s OR group_name ILIKE %s) AND era != 'Guess' AND category != 'guess_minigame' LIMIT 25",
        ...
        # Manually call autocomplete to catch and swallow discord.errors.NotFound (10062)
        try:
            await interaction.response.autocomplete(choices)
        except (discord.errors.NotFound, discord.errors.HTTPException):
            pass
        return [] # Return empty so the library doesn't try to respond again
    except Exception as e:
        if not isinstance(e, (discord.errors.NotFound, discord.errors.HTTPException)):
            print(f"[AUTOCOMPLETE ERROR] view: {e}")
        return []
