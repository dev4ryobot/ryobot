...
class RyoBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        try:
            # ⚡ DIRECT GUILD SYNC: Push directly to target guilds in GUILD_IDS for instant client-side command registration
            for g_id in GUILD_IDS:
                try:
                    guild = discord.Object(id=g_id)
                    self.tree.copy_global_to(guild=guild)
                    await self.tree.sync(guild=guild)
                    print(f"✅ Successfully synced commands to guild {g_id} on startup")
                except Exception as e:
                    print(f"⚠️ Could not sync commands to guild {g_id} on startup: {e}")

            # ⚡ GLOBAL SYNC: Push globally as fallback
            await self.tree.sync()
            print("✅ Slash commands synced globally as fallback.")
        except Exception as e:
