...
async def setup_database():
    ...
    queries = [
        ...
        "CREATE TABLE IF NOT EXISTS ryo_binder_templates (id TEXT PRIMARY KEY, name TEXT, image_url TEXT, paint_price INT DEFAULT 0, glue_price INT DEFAULT 0);",
...
@app_commands.describe(
    id="Unique ID/Number for the binder template",
...
async def add_binder_cmd(
    ...
    await run_query(
        ...
        """,
        (clean_id, name.strip(), clean_url, paint_price, glue_price)
    )
    await interaction.followup.send(f"✅ Successfully added/updated binder template `{clean_id}` ({name}) in the shop!", ephemeral=True)
...
class BinderShopSelect(discord.ui.Select):
    def __init__(self, templates):
        options = []
        for tid, tname, p_price, g_price, img_url in templates[:25]:
            prices = []
            if p_price and p_price > 0:
                prices.append(f"{p_price} Paint")
            if g_price and g_price > 0:
                prices.append(f"{g_price} Glue")
            price_str = " & ".join(prices) if prices else "Free"
            
            options.append(discord.SelectOption(
                label=tname[:100],
                value=tid,
                description=f"ID: {tid} | Cost: {price_str}"[:100],
                emoji="📖"
            ))
            
        super().__init__(
            placeholder="Select a binder theme to purchase...",
            min_values=1,
            max_values=1,
            options=options,
            custom_id="bindershop_select"
        )

    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.view.user_id:
            return await interaction.response.send_message("❌ This is not your shop menu. Please type `/bindershop` to open your own!", ephemeral=True)
        ...
        template = await run_query("SELECT id, name, paint_price, glue_price FROM ryo_binder_templates WHERE id = %s", (selected_id,), fetchone=True)
        if not template:
            return await interaction.followup.send("❌ That binder theme no longer exists in the shop.", ephemeral=True)
        ...
        if user_paint < total_paint_cost or user_glue < total_glue_cost:
            ...
            return await interaction.followup.send(f"❌ You do not have enough funds! Cost: {price_display}. You have {PAINT_EMOJI} {user_paint:,} Paint and {GLUE_EMOJI} {user_glue:,} Glue.", ephemeral=True)
...
class BinderShopView(discord.ui.View):
    def __init__(self, templates, user_id):
        super().__init__(timeout=180)
        self.user_id = user_id
        self.add_item(BinderShopSelect(templates))

@bot.tree.command(name="bindershop", description="View or purchase binders from the shop")
@app_commands.describe(
    binder_no="The ID/number of the binder template to buy",
    quantity="The number of binders to purchase"
)
async def bindershop_cmd(
    interaction: discord.Interaction,
    binder_no: str = None,
    quantity: int = 1
):
    ...
    if not binder_no:
        templates = await run_query("SELECT id, name, paint_price, glue_price, image_url FROM ryo_binder_templates ORDER BY id", fetchall=True) or []
        if not templates:
            embed = discord.Embed(
                title="🛍️ Binder Shop",
        ...
        embed = discord.Embed(
            title="🛍️ Binder Shop Catalog",
            description="Use `/bindershop [binder_no] [quantity]` to purchase a binder theme!\nOnce purchased, create a personalized binder instance using `/bindercreate`.",
            color=RYO_COLOR
        )
        
        for tid, tname, p_price, g_price, img_url in templates:
            prices = []
            if p_price and p_price > 0:
                prices.append(f"{PAINT_EMOJI} {p_price:,} Paint")
            if g_price and g_price > 0:
                prices.append(f"{GLUE_EMOJI} {g_price:,} Glue")
                
            price_str = " & ".join(prices) if prices else "Free"
            preview = f"[[Background Image]]({get_raw_url(img_url)})" if img_url and img_url.startswith("http") else "Classic Theme Background"
        ...
        embed.set_footer(text="RyoBot Binder Shop", icon_url=TEDDY_ICON_URL)
        view = BinderShopView(templates, interaction.user.id)
        return await interaction.followup.send(embed=embed, view=view)
        
    if quantity < 1:
        return await interaction.followup.send("❌ Quantity must be at least 1.")
        
    clean_id = binder_no.strip().lower()
    template = await run_query("SELECT id, name, paint_price, glue_price FROM ryo_binder_templates WHERE id = %s", (clean_id,), fetchone=True)
    if not template:
        return await interaction.followup.send(f"❌ Binder with ID `{clean_id}` does not exist in the shop.")
    ...
    if user_paint < total_paint_cost or user_glue < total_glue_cost:
        ...
        if total_glue_cost > 0: price_conditions.append(f"{GLUE_EMOJI} {total_glue_cost:,} Glue")
        price_display = " & ".join(price_conditions)
        return await interaction.followup.send(f"❌ You do not have enough funds! Cost: {price_display}. You have {PAINT_EMOJI} {user_paint:,} Paint and {GLUE_EMOJI} {user_glue:,} Glue.")
...
async def binderinv_cmd(interaction: discord.Interaction):
    ...
    rows = await run_query(
        """
        SELECT i.binder_id, t.name, i.quantity, t.image_url 
    ...
    if not rows:
        embed = discord.Embed(
            ...
            description="Your binder inventory is empty. Buy custom binder themes from the `/bindershop`!",
            color=RYO_COLOR
        )
        return await interaction.followup.send(embed=embed)
    ...
    embed = discord.Embed(
        ...
        description="These are the binder themes you purchased. Create active binders using `/bindercreate`!",
        color=RYO_COLOR
    )
    
    for bid, bname, qty, img_url in rows:
        preview = f"[[Preview Image]]({get_raw_url(img_url)})" if img_url and img_url.startswith("http") else "Classic Theme Background"
        embed.add_field(
            name=f"📖 {bname} (ID: `{bid}`)",
            value=f"**Quantity Owned**: {qty}\n{preview}",
            inline=False
        )
...
@bot.tree.command(name="bindercreate", description="Create a new custom card binder instance")
@app_commands.describe(
    binder_id="The ID of the purchased binder template to use from your inventory",
...
async def binder_create_cmd(
    ...
    await interaction.response.defer(ephemeral=True)
    uid = str(interaction.user.id)
    clean_bid = binder_id.strip().lower()
...
@binder_create_cmd.autocomplete('binder_id')
...
@bindershop_cmd.autocomplete('binder_no')
