...
async def get_binder_slots(binder_instance_id):
    rows = await run_query("SELECT slot, card_id FROM ryo_binder_slots WHERE binder_instance_id = %s", (binder_instance_id,), fetchall=True) or []
...
async def binderclear_cmd(interaction: discord.Interaction, binder_name: str):
    ...
    await run_query("DELETE FROM ryo_binder_slots WHERE binder_instance_id = %s", (instance_id,))
...
async def binderedit_cmd(
    ...
    if new_name:
        ...
        exists = await run_query("SELECT id FROM ryo_user_binders WHERE user_id = %s AND LOWER(name) = LOWER(%s) AND id != %s", (uid, clean_new_name, instance_id), fetchone=True)
        if exists:
    ...
    query = f"UPDATE ryo_user_binders SET {', '.join(updates)} WHERE id = %s"
...
async def binderdelete_cmd(interaction: discord.Interaction, binder_name: str):
    ...
    await run_query("DELETE FROM ryo_user_binders WHERE id = %s", (instance_id,))
    await run_query("DELETE FROM ryo_binder_slots WHERE binder_instance_id = %s", (instance_id,))
...
async def binder_add_cmd(
    ...
    for cid in set(valid_card_ids):
        row = await run_query(
            ...
            JOIN ryo_user_binders b ON s.binder_instance_id = b.id 
    ...
    filled_slots_rows = await run_query("SELECT slot, card_id FROM ryo_binder_slots WHERE binder_instance_id = %s", (instance_id,), fetchall=True) or []
    ...
    if slot is not None:
        ...
        for cid in valid_card_ids:
            ...
            await run_query(
                ...
                """,
                (instance_id, current_target_slot, cid)
            )
...
async def binder_remove_cmd(
    ...
    if slots_to_delete:
        ...
        res = await run_query(
            f"DELETE FROM ryo_binder_slots WHERE binder_instance_id = %s AND slot IN ({placeholders})",
            [instance_id] + list(slots_to_delete),
    ...
    if cards_to_delete:
        ...
        res = await run_query(
            f"DELETE FROM ryo_binder_slots WHERE binder_instance_id = %s AND LOWER(card_id) IN ({placeholders})",
            [instance_id] + list(cards_to_delete),
            return_rowcount=True
        )
        deleted_count += res
