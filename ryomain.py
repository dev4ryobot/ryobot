...
async def setup_database():
    ...
    # MIGRATION: Check if old ryo_user_binders table exists with old columns
    try:
        has_old_col = await run_query("""
            SELECT COLUMN_NAME 
            FROM INFORMATION_SCHEMA.COLUMNS 
            WHERE TABLE_NAME = 'ryo_user_binders' AND COLUMN_NAME = 'binder_id';
        """, fetchone=True)
        if has_old_col:
            # Drop the old tables to let the new setup recreate them perfectly
            ...
            await run_query("DROP TABLE IF EXISTS ryo_binder_inventory CASCADE;")
            await run_query("DROP TABLE IF EXISTS ryo_binder_templates CASCADE;")
            print("[Migration] Dropped old binder tables to migrate to the new binder system.")
    except Exception as e:
        print(f"[Migration Warning] Failed old binder tables migration: {e}")
    ...
    queries = [
        ...
        "CREATE TABLE IF NOT EXISTS ryo_binder_instances (binder_instance_id SERIAL PRIMARY KEY, user_id VARCHAR(50) NOT NULL, binder_number INT NOT NULL, is_active BOOLEAN DEFAULT FALSE, UNIQUE (user_id, binder_number));",
        "CREATE TABLE IF NOT EXISTS ryo_binder_slots (binder_instance_id INT REFERENCES ryo_binder_instances(binder_instance_id) ON DELETE CASCADE, slot INT CHECK (slot BETWEEN 1 AND 8), card_id VARCHAR(100) REFERENCES ryo_cards(card_id) ON DELETE SET NULL, PRIMARY KEY (binder_instance_id, slot));",
        "CREATE TABLE IF NOT EXISTS ryo_binder_shop_items (purchase_id VARCHAR(100) PRIMARY KEY, name VARCHAR(255) NOT NULL, drop_emoji VARCHAR(100) NOT NULL, item_type VARCHAR(100) NOT NULL, image_url TEXT NOT NULL, paint_price INT DEFAULT 0, glue_price INT DEFAULT 0);",
        "CREATE TABLE IF NOT EXISTS ryo_user_binders (user_id VARCHAR(50) NOT NULL, purchase_id VARCHAR(100) NOT NULL, purchased_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (user_id, purchase_id));",
...
class BinderShopView(discord.ui.View):
    ...
    async def on_buy_click(self, interaction: discord.Interaction):
        ...
        current_item = self.items[self.current_index]
        purchase_id, name, _, _, _, paint_price_raw, glue_price_raw = current_item
        paint_price = paint_price_raw or 0
        glue_price = glue_price_raw or 0
        # Check ownership
...
def build_bindershop_embed(interaction, items, current_index):
    ...
    purchase_id, name, drop_emoji, item_type, image_url, paint_price_raw, glue_price_raw = current_item
    paint_price = paint_price_raw or 0
    glue_price = glue_price_raw or 0
    # Format prices
    prices = []
