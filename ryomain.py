...
async def refundlist_cmd(interaction: discord.Interaction):
    ...
    def make_embed(page_idx):
        embed = discord.Embed(
            ...
            description=f"**{len(rows)} card(s)** currently in the refundable list.",
            color=RYO_COLOR
        )
        lines = []
        for card_id, member, group, era, rarity, category in pages[page_idx]:
        ...
        embed.set_footer(text=f"Page {page_idx + 1}/{total_pages} • Total: {len(rows)}")
        return embed
    ...
    view = RefundListView() if total_pages > 1 else None
    if view:
