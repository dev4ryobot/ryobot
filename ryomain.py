...
@app_commands.describe(name="Idol Name", image_url="Direct image link")
async def addgti(interaction: discord.Interaction, name: str, image_url: str = None):
    ...
    await addcard_logic(interaction, card_id, name, "Guess", "Solo", "guess_minigame", image_url, 1, True)

@bot.tree.command(name="addgtg", description="Add a new Guess The Group card (STAFF/MOD ONLY)")
@app_commands.describe(name="Group Name", image_url="Direct image link")
async def addgtg(interaction: discord.Interaction, name: str, image_url: str = None):
    ...
    await addcard_logic(interaction, card_id, "Group", "Guess", name, "guess_minigame", image_url, 1, True)
...
async def shop_view(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title="🏪 Ryo Staff Pack Shop", 
        description="Welcome to the Pack Shop! Purchase custom card packs using your **Paint** balance.\n",
        color=RYO_COLOR
    )
    
    # Check if user exists and fetch their Paint balance for convenience
    uid = str(interaction.user.id)
    balance_row = await run_query("SELECT paint FROM ryo_users WHERE user_id = %s", (uid,), fetchone=True)
    current_paint = balance_row[0] if balance_row else 0
    
    embed.add_field(
        name="🎨 Your Current Balance",
        value=f"🎨 `{current_paint:,}` Paint",
        inline=False
    )
    ...
    for p in packs:
        ...
        embed.add_field(
            name=f"📦 {name} (ID: `{pack_id}`)",
            value=f"**Price:** 🎨 `{price:,}` Paint\n*{description}*",
            inline=False
        )
        
    embed.set_footer(text="Use /shopbuy [id] [quantity] [optional-group] to purchase!")
...
async def shop_buy(interaction: discord.Interaction, id: str, quantity: int, group: str = None):
    ...
    # Check balances
    user_row = await run_query("SELECT paint FROM ryo_users WHERE user_id = %s", (buyer_id,), fetchone=True)
    ...
    if user_paint < total_cost:
        return await interaction.followup.send(
            ...
            f"Ordering {quantity}x `{pack_name}` costs **🎨 {total_cost:,} Paint**.\n"
            f"Your current balance is **🎨 {user_paint:,} Paint**."
        )
    ...
    if is_group_pack:
        ...
        group_cards = await run_query(
            ...
            "WHERE LOWER(group_name) = LOWER(%s) AND LOWER(category) != 'guess_minigame' AND LOWER(era) != 'guess'",
            (group.strip(),),
            fetchall=True
        )
        ...
        r5_cards = await run_query(
            "SELECT card_id, member_name, group_name, era, rarity, image_url, category FROM ryo_cards "
            "WHERE rarity = 5 AND LOWER(category) != 'guess_minigame' AND LOWER(era) != 'guess'",
            fetchall=True
        )
        ...
        all_draw_cards = await run_query(
            "SELECT card_id, member_name, group_name, era, rarity, image_url, category FROM ryo_cards "
            "WHERE LOWER(category) != 'guess_minigame' AND LOWER(era) != 'guess'",
            fetchall=True
        )
    ...
    embed = discord.Embed(
        ...
        description=f"<@{buyer_id}> bought **{quantity}x** `{pack_name}` for **🎨 {total_cost:,} Paint**!\n\n"
