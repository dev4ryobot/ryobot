...
async def setup_database():
    ...
    queries = [
        ...
        "INSERT INTO ryo_binder_templates (id, name, image_url, paint_price, glue_price) VALUES ('classic', 'Classic Theme', 'local_fallback', 0, 0) ON CONFLICT (id) DO NOTHING;",
...
async def shop_view(interaction: discord.Interaction):
    packs = await run_query("SELECT pack_id, name, description, price, glue_price FROM ryo_shop_packs ORDER BY price ASC, glue_price ASC", fetchall=True)
    if not packs:
        return await interaction.response.send_message(
