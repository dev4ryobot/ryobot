...
@app_commands.describe(mode="Choose sync type")
async def sync(interaction: discord.Interaction, mode: Literal["guild", "global"]):
    ...
    try:
        if mode == "global":
            ...
            await interaction.followup.send(
                f"🌍 Synced {len(synced)} globally (may take time)"
            )
        else:
            ...
            await interaction.followup.send(
                f"⚡ Synced {len(synced)} to this server!"
            )
